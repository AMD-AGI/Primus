...
e4m3_type = tex.DType.kFloat8E4M3
...
class TEGemmRS:
    def __init__(
        ...
        if parallel_mode == "row":
            # fwd: output -> rs
            self.input_shape = (mbs * m, k // TP_SIZE)
            ...
            # bwd: dgrad -> rs
            self.input_shape = (mbs * m, n // TP_SIZE)
    ...
    def prepare_input_output(self):
        ...
        rs_out = torch.empty(
            (self.ub_shape[0] // self.tp_size, self.ub_shape[1]), dtype=self.output_dtype, device=self.device
        )
        if self.use_fp8:
            num_gemms = 3
            fp8_dtype = e4m3_type
            ...
            return inp_fp8, weight_fp8, rs_out
        return input, weight, rs_out

    def prepare_ub(self):
        self.ub_obj = tex.CommOverlap(
            self.ub_shape,  # Communication buffer shape
            self.output_dtype,  # Communication buffer data type
            self.tp_group.group_name,
            # Tensor-parallel group size (may be different than local_size)
            self.tp_size,
            num_splits=self.num_splits,
        )
        self.comm_type = tex.CommOverlapType.RS

    def run(self, inp, weight, rs_out, is_opt):
        if is_opt:
            assert self.ub_obj is not None and self.comm_type is not None and rs_out is not None
        ...
        output, *_, rs_out = te.cpp_extensions.general_gemm(
            ...
            extra_output=rs_out if is_opt else None,
            bulk_overlap=False,
        )

        if not is_opt:
            rs_out, _ = te.distributed.reduce_scatter_along_first_dim(output, self.tp_group)
        return rs_out


def titan_base_scaled_gemm_rs(
    A: torch.Tensor, B: torch.Tensor, kwargs: dict[str, Any], TP_GROUP
) -> torch.Tensor:
    if kwargs is None or kwargs.get("A_scale") is None:
        output = reduce_scatter_tensor(A @ B, "sum", 1, TP_GROUP)
    else:
        ...
        output = reduce_scatter_tensor(C, "sum", 1, TP_GROUP)
    wait_tensor(output)
    return output


def titan_compile_gemm_rs(
    func, A: torch.Tensor, B: torch.Tensor, kwargs: dict[str, Any], TP_GROUP, debug_mode=False
) -> torch.Tensor:
    ...
    code = run_and_get_triton_code(compiled, A, B, kwargs, TP_GROUP)
    assert (
        "fused_matmul_reduce_scatter" not in code
        and "fused_scaled_matmul_reduce_scatter" not in code
        and "reduce_scatter_tensor" in code
    ), "Async TP applied in gemm rs ops not as we expected."

    return compiled


def titan_overlap_gemm_rs(
    func, A: torch.Tensor, B: torch.Tensor, kwargs: dict[str, Any], TP_GROUP, debug_mode=False
):
    ...
    code = run_and_get_triton_code(compiled, A, B, kwargs, TP_GROUP)
    assert (
        "fused_matmul_reduce_scatter" in code or "fused_scaled_matmul_reduce_scatter" in code
    ), "Async TP not applied in gemm rs ops"
...
def titan_orig_gemm_rs(
    ...
    restore_async_tp()
    return titan_overlap_gemm_rs(func, A, B, kwargs, TP_GROUP, debug_mode)
...
def benchmark_titan(report_dir_path, model_config, profile, fp8, TP_GROUP, TP_SIZE, local_rank):
    ...
    for dtype, mbs, shape_dims, use_fp8 in tqdm(
        param_combos, desc=f"TiTan {model_name} Gemm_RS_Overlap Benchmarking"
    ):
        shape, parallel_mode = shape_dims
        m, n, k = shape
        if parallel_mode == "row":
            # fwd: output -> rs
            A_shape = (mbs, m, k // TP_SIZE)
            B_shape = (k // TP_SIZE, n)
        else:
            # bwd: dgrad -> rs
            A_shape = (mbs, m, n // TP_SIZE)
            B_shape = (n // TP_SIZE, k)
        if use_fp8:
            A = torch.randn(A_shape, device=device).to(e4m3_type)
            B = torch.randn(B_shape, device=device).to(e4m3_type).t().contiguous().t()
            ...
            A = torch.randn(A_shape, device=device).to(dtype)
            B = torch.randn(B_shape, device=device).to(dtype)
            kwargs = None

        base_func = titan_base_scaled_gemm_rs
        compile_func = titan_compile_gemm_rs(base_func, A, B, kwargs, TP_GROUP, True)
        ...
        with ctx:
            base_output, base_time = perf_func(
                partial(base_func, A, B, kwargs, TP_GROUP),
                iters=NUM_ITERS,
            ...
            compile_output, compile_time = perf_func(
                partial(compile_func, A, B, kwargs, TP_GROUP),
                iters=NUM_ITERS,
                warmup_iters=NUM_WARMUP,
            )
            ...
            orig_overlap_func = titan_orig_gemm_rs(base_func, A, B, kwargs, TP_GROUP, True)
            orig_overlap_output, orig_overlap_time = perf_func(
                partial(orig_overlap_func, A, B, kwargs, TP_GROUP),
                iters=NUM_ITERS,
                warmup_iters=NUM_WARMUP,
            )
            ...
            patch_overlap_func = titan_patch_gemm_rs(base_func, A, B, kwargs, TP_GROUP, True)
            patch_overlap_output, patch_overlap_time = perf_func(
                partial(patch_overlap_func, A, B, kwargs, TP_GROUP),
                iters=NUM_ITERS,
                warmup_iters=NUM_WARMUP,
            )
            torch.cuda.synchronize()
        ...
        if profile:
            run_id = f"titan_tp{TP_SIZE}_b{mbs}_m{m}_n{n}_k{k}_{parallel_mode}_dtype_{dtype}_fp8_{use_fp8}"
...
def benchmark_megatron(report_dir_path, model_config, profile, fp8, TP_GROUP, TP_SIZE, local_rank):
    ctx = get_torch_prof_ctx(profile)
    device = torch.device(f"cuda:{local_rank}")
    model_name = model_config["model"]
    seq = model_config["seqlen"]
    ...
    for dtype, mbs, shape_dims, use_fp8 in tqdm(
        param_combos, desc=f"Megatron {model_name} Gemm_RS_Overlap Benchmarking"
    ):
        shape, parallel_mode = shape_dims
        m, n, k = shape
        te_gemm_rs = TEGemmRS(
        ...
        input, weight, rs_out = te_gemm_rs.prepare_input_output()
        ...
        with ctx:
            te_restore()
            base_output, base_time = perf_func(
                partial(te_gemm_rs.run, input, weight, rs_out, False),
                iters=NUM_ITERS,
                warmup_iters=NUM_WARMUP,
            )
            ...
            te_patch()
            te_gemm_rs.prepare_ub()
            overlap_output, overlap_time = perf_func(
                partial(te_gemm_rs.run, input, weight, rs_out, True),
                iters=NUM_ITERS,
                warmup_iters=NUM_WARMUP,
            )
...
def benchmark(args):
    ...
    for model_config in model_config_list:
        ...
        if args.backend == "torchtitan":
            benchmark_titan(
                args.report_dir_path, model_config, args.profile, args.fp8, TP_GROUP, TP_SIZE, local_rank
            )
