...
> **Grouped GEMM keys.** `use_turbo_grouped_mlp` is a **deprecated alias** for `use_turbo_grouped_gemm` (the trainer prints a deprecation warning and treats them identically); the example scripts still pass `--use_turbo_grouped_mlp True`, but prefer `--use_turbo_grouped_gemm True` in new configs.
>
...
`turbo_sync_free_moe_stage` is a single knob with four levels (`0`–`3`, validated in `primus/modules/trainer/megatron/utils.py`). Each stage **auto-enables** a set of fusion flags:
