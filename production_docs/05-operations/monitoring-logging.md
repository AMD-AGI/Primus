...
- `WANDB_API_KEY` is **required** when Weights & Biases is enabled; the Megatron trainer emits a warning if it is missing (`primus/modules/trainer/megatron/trainer.py`).
