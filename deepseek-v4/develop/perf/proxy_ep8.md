...
| **P41**        | `Indexer.forward` post-einsum tail Triton FWD/BWD fusion (**DESCOPED**, default OFF) | **513.9** | **523.7** | **17.20x** | 10-iter smoke `progress/p41/`; eager-tail A side mean iters 8/9/10 = **513.7 ms**, P41 Triton-tail B side = **513.9 ms** → +0.2 ms within ±1 ms noise; lm_loss bit-identical iter-by-iter. Microbench wins at V4-Flash (FWD 4.30x / BWD 1.63x) do not surface in the proxy because the per-CSA-layer scope (3 of 8 layers) is below the dispatch-noise floor. Default OFF; opt-in via `PRIMUS_INDEXER_TRITON=1`. Legacy P38 full-fuse path moved to `PRIMUS_INDEXER_TRITON_FULL=1` (also default OFF). |
| **P42**        | Permute + `.contiguous()` absorption into V4 attention / CSA FWD inputs (**DESCOPED at task-list refinement**) | n/a | n/a | n/a | `progress/p42/p42-summary.md`. Trace-attribution shows top-5 eliminate candidates account for ~0.7 ms / iter (below noise); remaining ~97 % of `bf16_copy` launches are TE / Apex / DeepEP machinery (out-of-model). High-risk plan-5 kernel modification deferred to plan-8 attention re-work. |
| **P43**        | V4 router post-logits 50-iter × 3-run A/B re-attempt (**DESCOPE REAFFIRMED**, default OFF) | **509.55** (eager A) / **510.17** (triton B, regression) | **528.0** | **17.35x** | `progress/p43/`. n=135 samples per side, noise floor ±0.19 ms; delta B-A = **+0.62 ms** (Triton **slower** than eager, 3.2× noise — statistically significant). Microbench predicted +1.5 ms / iter gain at V4-Flash on `sqrtsoftplus`; proxy shows -0.62 ms net (2.1 ms swing unattributed; likely Inductor fusion in eager body). `PRIMUS_V4_ROUTER_TRITON` stays `"0"`. Compressor APE elementwise fusion sub-task descoped at task-list refinement (~3-4 ms / iter savings below the 10 % cut-off). |
| **P44**        | V4 attention FWD epilogue (`out * scale + sinks`) — **DESCOPED at task-list refinement** | n/a | n/a | n/a | `progress/p44/p44-summary.md`. Originally-scoped target does not map to existing code — `attn_sink` is already absorbed via virtual softmax row in `_v4_attention_fwd_kernel:289-295`; `softmax_scale` is already a kernel parameter. Trace bucket `vec_elem<mul_bf16>` AUnary 5.71 ms (1.1 %) is unattributed + below R9.1 10 % cut-off. |
| **P45**        | Triton multi-tensor BF16 add prototype (plan-7; **production integration descoped to plan-8**) | n/a | n/a | n/a | `progress/p45/p45-summary.md`. Microbench evidence at n=743 (matches trace launch count): `torch._foreach_add_` already runs at ~2000 GB/s (near MI355 peak); Triton per-tensor variant ties at 1.02x. The 743 `vec_elem<add_bf16>` launches are NOT launch-overhead-dominated. Real fused-Adam win requires master-param remainder bit-exactness (plan-8). |
...
| **P57 R2 close-out 2** | Tilelang optional + CLI-driven; Indexer/Router Triton default ON | **436.80** (best 432.6) | **613.6** (best 619.4) | **20.23x** (best 20.43x) | `progress/p57/runs/p57r2_no_tilelang_v2_proxy_trace.log` (10-iter trace; iters 4-6+8-10 mean; iter 7 excluded). Validates 3 changes vs the previous P57 row, none of which regress wall-clock (+0.67 ms within ±5 ms noise; throughput -0.9 TFLOP/s within ±10 TFLOP/s noise): (1) **tilelang is now opt-in via two CLI flags** `--use_v4_tilelang_attention` / `--use_v4_tilelang_csa_attention` (default-off) instead of the legacy `PRIMUS_V4_TILELANG_ATTN=1` env knob; containers without tilelang installed (the default Primus container) leave both flags off and the dispatcher never tries to import tilelang. (2) `PRIMUS_INDEXER_TRITON` default flipped from `0` to `1` -- microbench positive (FWD 4.30x / BWD 1.63x) with a small positive proxy A/B (~0.2 ms / iter) that previously sat below the noise floor and kept the kernel descoped to default-OFF. (3) `PRIMUS_V4_ROUTER_TRITON` default flipped from `0` to `1` -- microbench positive on V4's `sqrtsoftplus` (1.56x FWD / 1.22x BWD); P39 / P43 proxy A/B sat inside the noise band so we move the microbench-positive kernel onto the production path. `lm_loss[10]` = 9.224617 vs P57 (no-router-triton) 9.224450 (-0.000167 / 2e-5; within bf16 noise floor). |
