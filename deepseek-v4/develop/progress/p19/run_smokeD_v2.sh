#!/bin/bash
# Smoke D v2: PP=2 EP=4 VPP=2 with the V4 hash-router input_ids broadcast
# moved out of DeepseekV4Model.forward into a primus-patch on
# pretrain_gpt.get_batch (deepseek_v4_get_batch_patches.py). Validates that
# the new hook does not deadlock the interleaved-1F1B / VPP schedule.
set -euo pipefail
set -x
...
export PRIMUS_EXP_NAME=p19_smokeD_pp2_ep4_vpp2_v2
...
./primus-cli direct \
  ...
  --num_virtual_stages_per_pipeline_rank "$PRIMUS_VPP" \
