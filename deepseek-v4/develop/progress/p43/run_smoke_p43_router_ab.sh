#!/bin/bash
# Plan-6 P43 — 50-iter EP=8 proxy smoke, no profiler.  A/B between
# `PRIMUS_V4_ROUTER_TRITON=0` (eager body, P40 production) and `=1`
# (P39 Triton kernel).  Designed to be run 3 times per side and
# aggregated by `progress/p43/aggregate_router_ab.py` -- the
# noise-reduction goal is to defeat the ~±1 ms NCCL / dispatch
# variance that swamped the original P39 10-iter A/B.
set -euo pipefail
set -x
...
_P43_TAG="p43_smoke_router_triton_${PRIMUS_V4_ROUTER_TRITON}_run${P43_RUN_TAG:-1}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P43_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
