#!/bin/bash
# Plan-5 P30 — G34/G33b-style perf trace after dense v4_attention SWA
# K-loop pruning.
#
# Same proxy and profiler window as P29:
#   TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7.
#
# Single delta vs P29:
#   dense (cr == 0) V4 Triton attention routes through the kernel's
#   `swa_window > 0` path, and the kernel skips K tiles that are fully
#   outside the sliding window.  HCA still uses the joint additive-mask
#   path; its structural LSE-merge rewrite remains tracked separately.
#
# Output:
#   output/<team>/<user>/p30_profile_swa_prune_pp1_ep8_seq4096/tensorboard/
...
export PRIMUS_EXP_NAME=p30_profile_swa_prune_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}
