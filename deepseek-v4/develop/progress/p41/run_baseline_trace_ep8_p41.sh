#!/bin/bash
# Plan-6 P41 — perf trace after the Indexer post-einsum tail Triton
# kernel (`indexer_score_post.py`) replaces the eager
# `relu + mul + sum + causal_mask` body in `Indexer.forward`.
#
# Same proxy + profiler window as the P40 close-out trace
# (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7).
#
# A side (eager tail): PRIMUS_INDEXER_TRITON=0
# B side (Triton tail): PRIMUS_INDEXER_TRITON=1 (this script's default)
set -euo pipefail
set -x
...
# P41: post-einsum tail Triton; flip to "0" to run the eager-tail A side.
...
_P41_TAG="p41_profile_indexer_tail_${PRIMUS_INDEXER_TRITON}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P41_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
