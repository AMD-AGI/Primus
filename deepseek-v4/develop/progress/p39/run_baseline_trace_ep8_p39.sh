#!/bin/bash
# Plan-6 P39 — perf trace after the V4 router post-logits Triton FWD/BWD
# kernel replaces the eager scoring / gather / sum.clamp.div / scaling /
# scatter chain in both ``v4_topk_router.py`` and ``v4_hash_router.py``.
#
# Same proxy and profiler window as the P32 final / P34 / P35 / P36 / P37
# traces (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7).  The deltas
# captured here vs the P37 trace are:
#
#   PRIMUS_STACK_GROUPED_WEIGHT_TRITON=1   (carried forward from P34)
#   PRIMUS_ROPE_TRITON=1                    (carried forward from P35)
#   PRIMUS_SINKHORN_TRITON=1                (carried forward from P36)
#   PRIMUS_HC_TRITON=1                      (carried forward from P37)
#   PRIMUS_INDEXER_TRITON=0                 (descoped at P38; default OFF)
#   PRIMUS_V4_ROUTER_TRITON=1               (new P39 default)
#
# Expected delta in the chrome trace vs the P37 trace:
#
#   * Disappearance of ~7 elementwise / scatter launches per router
#     call (score_fn + gather + sum.clamp.div + 2 scatters).
#   * New kernel names in the trace:
#     - ``_v4_router_post_fwd_kernel`` (one per MoE layer / iter,
#       ~16 per iter at 8 layers / 2 microbatch steps).
#     - ``_v4_router_post_bwd_kernel`` (matching BWD).
#   * Iter time delta: ~0.5 ms / iter (FWD+BWD savings on
#     `sqrtsoftplus`).  Modest, but bit-identical on lm_loss and is
#     the structural prerequisite for fusing the dispatcher input
#     downstream (out of plan-6 scope).
#
# To capture the A side (eager router fallback), set:
#   PRIMUS_V4_ROUTER_TRITON=0  bash deepseek-v4/develop/progress/p39/run_baseline_trace_ep8_p39.sh
set -euo pipefail
set -x
...
# ---------- Plan-6 P35 knob (carried forward) -------------------------------
export PRIMUS_ROPE_TRITON="${PRIMUS_ROPE_TRITON:-1}"

# ---------- Plan-6 P36 knob (carried forward) -------------------------------
export PRIMUS_SINKHORN_TRITON="${PRIMUS_SINKHORN_TRITON:-1}"

# ---------- Plan-6 P37 knob (carried forward) -------------------------------
export PRIMUS_HC_TRITON="${PRIMUS_HC_TRITON:-1}"

# ---------- Plan-6 P38 knob (descoped; default OFF) -------------------------
export PRIMUS_INDEXER_TRITON="${PRIMUS_INDEXER_TRITON:-0}"

# ---------- Plan-6 P39 knob (descoped; default OFF) -------------------------
# Microbench wins (1.56x FWD / 1.22x BWD on sqrtsoftplus) didn't surface in
# the EP=8 proxy A/B -- lm_loss bit-identical, iter-time within ~1-3 ms
# noise.  Set =1 to capture the Triton-on side of the A/B.
...
_P39_TAG="p39_profile_router_triton_${PRIMUS_V4_ROUTER_TRITON}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P39_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
