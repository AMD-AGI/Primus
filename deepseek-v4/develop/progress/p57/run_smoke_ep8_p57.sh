#!/bin/bash
# Plan-8 P57 — Triton V4 attention perf-push proxy smoke (no profiler).
#
# Same widths as the trace runner but PROFILE=False, so iter time is the
# clean post-warmup steady state (use this for ratchet numbers and to
# compare apples-to-apples against P40 final 510.6 ms / iter).
set -euo pipefail
set -x
...
export TRAIN_ITERS=${TRAIN_ITERS:-15}
...
export PRIMUS_EXP_NAME=p57_smoke_triton_attn_perf_push_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}
