#!/bin/bash
# Plan-6 P40 — plan-6 close-out perf trace.
#
# Same proxy and profiler window as the P32 final / P34..P39 traces
# (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7).  This trace pins
# the cumulative plan-6 production state: all four default-on knobs
# enabled (P34 / P35 / P36 / P37) and the two descoped knobs off
# (P38 / P39).
#
#   PRIMUS_STACK_GROUPED_WEIGHT_TRITON=1   (P34; carried forward)
#   PRIMUS_ROPE_TRITON=1                    (P35; carried forward)
#   PRIMUS_SINKHORN_TRITON=1                (P36; carried forward)
#   PRIMUS_HC_TRITON=1                      (P37; carried forward)
#   PRIMUS_INDEXER_TRITON=0                 (P38; descoped, default OFF)
#   PRIMUS_V4_ROUTER_TRITON=0               (P39; descoped, default OFF)
#
# Companion close-out analysis lives at:
#   - `develop/profile/profile-after-p40-ep8-<YYYYMMDD>.md`
#   - `develop/progress/p40/p40-summary.md`
set -euo pipefail
set -x
...
# ---------- Plan-6 P38 / P39 (descoped; default OFF) ------------------------
...
_P40_TAG="p40_profile_plan6_close"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P40_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
