#!/bin/bash
# Plan-6 P37 — perf trace after the HyperConnection compute_weights tail
# Triton FWD/BWD kernel replaces the eager body (3 slices + 3 FMAs +
# 2 sigmoid + 1 softmax + 2 eps adds) between ``_packed_logits`` and
# ``sinkhorn_normalize``.
#
# Same proxy and profiler window as the P32 final / P34 / P35 / P36
# traces (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7). The deltas
# captured here vs the P36 trace are:
#
...
#   PRIMUS_HC_TRITON=1                      (new P37 default)
#
# Expected delta in the chrome trace vs the P36 trace:
#
#   * Disappearance of ~8 ``elementwise_kernel`` launches per
#     HyperConnection call inside compute_weights tail.
#   * New kernel name in the trace:
#     ``_hc_compute_tail_fwd_kernel`` (one per HyperConnection call,
#     ~16 per iter at 8 layers) plus a matching ``_bwd_kernel``.
#   * Iter time: ~440-480 ms (P36) -> ~435-475 ms (~3-5 ms / iter
#     reduction from the elemwise tail collapse).
#
# To capture the A side (eager tail fallback), set:
#   PRIMUS_HC_TRITON=0  bash deepseek-v4/develop/progress/p37/run_baseline_trace_ep8_p37.sh
set -euo pipefail
set -x
...
_P37_TAG="p37_profile_hc_triton_${PRIMUS_HC_TRITON}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P37_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
