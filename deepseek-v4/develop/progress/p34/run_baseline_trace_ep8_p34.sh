#!/bin/bash
# Plan-6 P34 — perf trace after the stack_grouped_weight Triton kernel
# replaces the eager torch.stack + transpose + contiguous chain in
# PrimusTurboGroupedMLP._stack_grouped_linear_weight.
#
# Same proxy and profiler window as the P32 final trace
# (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7). The delta vs
# P32 final is a single env flag:
#
#   PRIMUS_STACK_GROUPED_WEIGHT_TRITON=1
#
# Expected delta in the chrome trace vs the P32 final trace:
#
#   * ``hipMemcpyWithStream`` bucket:  289.6 ms / 32 calls  ->  ≤ 80 ms / ~0 calls
#     (the bucket may disappear entirely; the kernel writes the [E, N, K]
#     output in a single pass, so the secondary contiguous() memcpy that
#     dominated the bucket goes away).
#   * New kernel name in the trace: ``_stack_grouped_weight_fwd_kernel``
#     and ``_stack_grouped_weight_bwd_kernel``.
#   * Iter time: 603 ms  ->  ~430-450 ms.
#
# To run the P32-final A side (env flag off), set
# PRIMUS_STACK_GROUPED_WEIGHT_TRITON=0 before launching this script.
set -euo pipefail
set -x
...
# P32 final BWD optimisations stay ON (set via env)
...
_P34_TAG="p34_profile_stack_grouped_weight_triton_${PRIMUS_STACK_GROUPED_WEIGHT_TRITON}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P34_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
