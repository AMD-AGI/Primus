#!/bin/bash
# Plan-6 P36 — perf trace after the Sinkhorn-Knopp Triton FWD/BWD kernel
# replaces the plan-5 P29 ``torch.compile`` cached body in
# `primus.backends.megatron.core.transformer.hyper_connection.sinkhorn_normalize`.
#
# Same proxy and profiler window as the P32 final / P34 / P35 traces
# (TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7). The deltas captured
# here vs the P35 trace are:
#
#   PRIMUS_STACK_GROUPED_WEIGHT_TRITON=1   (carried forward from P34)
#   PRIMUS_ROPE_TRITON=1                    (carried forward from P35)
#   PRIMUS_SINKHORN_TRITON=1                (new P36 default)
#
# Expected delta in the chrome trace vs the P35 trace:
#
#   * ``Torch-Compiled Region`` (FWD-side Sinkhorn) ~21 ms / 16 calls -> ≈ 0
#     (the cached compiled artefact is bypassed entirely).
#   * ``CompiledFunctionBackward`` (Sinkhorn BWD) ~41 ms / 16 calls -> ≈ 0.
#   * New kernel names in the trace:
#     ``_sinkhorn_fwd_kernel`` / ``_sinkhorn_bwd_kernel`` (one each per
#     HyperConnection call, i.e. ~16 per iter at 8 layers).
#   * Iter time: ~480-510 ms (P35) -> ~440-480 ms (up to ~60 ms saved
#     when the compiled-region overhead is fully removed).
#
# To capture the A side (compiled fallback), set:
#   PRIMUS_SINKHORN_TRITON=0  bash deepseek-v4/develop/progress/p36/run_baseline_trace_ep8_p36.sh
set -euo pipefail
set -x
...
_P36_TAG="p36_profile_sinkhorn_triton_${PRIMUS_SINKHORN_TRITON}"
export PRIMUS_EXP_NAME="${PRIMUS_EXP_NAME:-${_P36_TAG}_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}}"
