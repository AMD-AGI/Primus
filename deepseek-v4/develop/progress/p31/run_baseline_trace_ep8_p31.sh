#!/bin/bash
# Plan-5 P31 — perf trace after CSA in-kernel top-K gather/scatter.
#
# Same proxy and profiler window as P30:
#   TP=1 PP=1 EP=8, Sq=4096, 8 layers, iter 6 -> 7.
# Delta vs P30:
#   cr=4 CSA layers pass pool + topk_idxs to the Triton kernel and avoid
#   wrapper-side gathered tensor materialisation and its autograd scatter.
set -euo pipefail
set -x
...
export PRIMUS_EXP_NAME=p31_profile_csa_in_kernel_gather_pp${PRIMUS_PP}_ep${PRIMUS_EP}_seq${PRIMUS_SEQ_LENGTH}
