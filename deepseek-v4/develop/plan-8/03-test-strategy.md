...
| **G57** | P57 | runtime (GPU) | Microbench wall-clock targets at V4-Flash widths (`B=1, H=64, Sq=4096, D=512, swa_window=128, sink=True, bf16`): **cr=0 BWD ≤ 3.0 ms**, **cr=4 FWD ≤ 1.5 ms**, **cr=4 BWD ≤ 5.0 ms**, **cr=128 BWD ≤ 3.0 ms**.  Bench harnesses: `progress/p32/bench_v4_attention_ep8.py --mode {dense,hca}` for cr=0/128, `progress/p31/bench_csa_attention_ep8.py` for cr=4.  Each kernel modification MUST keep the plan-4 G23/G24/G26/G27 parity tests green before the kernel ships as default-on. | `tests/unit_tests/megatron/transformer/deepseek_v4/test_v4_p25_v4_attention_{fwd,bwd}.py` + `test_v4_p26_v4_csa_attention_{fwd,bwd}.py` + the bench scripts |
| **G57a** | P57 | smoke + perf | EP=8 10-iter proxy smoke with the P57 kernel set default-on; plan-4..plan-8 ratchet stays green; `lm_loss` within `5e-2` of the plan-7 P48 baseline (510.6 ms / iter / 9.257534 lm_loss at iter 10).  No banned warnings; no NaN / Inf. | `progress/p57/run_smoke_p57_ep8.sh` |
| **G57b** | P57 | perf docs | `develop/perf/attention_perf.md` — append a P57 row using the R2.5 `<ms> ms \| <tflops>` format with the landed wall-clock numbers.  Per-target deltas vs P32 RoPE-fix final documented in `p57-summary.md` §4. | `develop/perf/attention_perf.md` + `progress/p57/p57-summary.md` |
