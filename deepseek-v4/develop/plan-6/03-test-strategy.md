...
green. Plan-6 adds G36..G46 (G43 added at P41 reopening; G44/G45/G46
added at P42/P43/P44 reopening).
...
| **G43** | P41 | runtime (GPU) | Triton `Indexer` post-einsum tail FWD `scores` parity vs the eager **tail** (extracted helper that runs `relu + mul + sum(H) + causal mask` on the same `dot [B, S, H, P]`) within bf16 `atol=5e-3 rtol=5e-3`. Post-`topk` `topk_idxs` bit-equal vs eager (load-bearing — sparse top-K indices must be exact, same contract as G41). BWD `gradcheck` fast tier fp32 small shape. Release-tier shape (`B=1, S=4096, P=1024, H=8, Hd=128, bf16`) marked `pytest.mark.slow`. Parametrise `compress_ratio ∈ {1, 4, 16}` so the causal mask geometry is exercised at the boundary cases. | `tests/unit_tests/megatron/transformer/deepseek_v4/test_p41_indexer_tail_triton.py` |
| **G43a** | P41 | smoke + perf | TP=1 PP=1 EP=8 10-iter proxy smoke under `run_deepseek_v4_flash_proxy.sh` with `PRIMUS_INDEXER_TRITON=1` (the **re-purposed** flag — P41 tail-only path, not the legacy P38 full-fuse).  Plan-4 ratchet (G23..G30) + plan-5 G32 / G34 / G34b / G35 + plan-6 G36..G42 stay green.  `lm_loss` after 10 iters within `5e-2` of the post-P40-final baseline at fixed seed.  Banned-warning grep returns 0. | `progress/p41/run_smoke_p41_ep8.sh` |
| **G43b** | P41 | perf + report | Capture chrome-trace iter 6 → 7 with `PRIMUS_INDEXER_TRITON=1`; render `develop/profile/profile-after-p41-ep8-<YYYYMMDD>.{md,html}`.  Phase-specific perf gate: Indexer scoring elementwise chain (relu / mul / sum / mask alloc / mask add) ≈ 0 in the trace.  Plus: end-to-end iter time drops by ≥ 2 ms / iter vs P40 final (510.6 ms mean iters 8-15).  **If proxy A/B regresses, `PRIMUS_INDEXER_TRITON` default stays `"0"`** and the regression goes in `progress/p41/p41-summary.md` "failed / negative probes" section. | `progress/p41/run_baseline_trace_ep8_p41.sh` + `develop/profile/profile-after-p41-ep8-<YYYYMMDD>.{md,html}` |
| **G44** | P42 | runtime (GPU) | V4 attention / CSA FWD with strided / pre-permute inputs FWD parity vs eager `.contiguous() + kernel` within bf16 `atol=1e-3 rtol=1e-3` at fast tier (`B=2 S=128 H=4 head_dim=8`) and release tier (`B=1 S=4096 H=64 head_dim=512`, `pytest.mark.slow`).  BWD `gradcheck` fp32 fast tier; BWD bf16 parity at release tier.  Parametrise `permute_pattern ∈ {dense_q, dense_kv, hca_q, csa_gathered}`. | `tests/unit_tests/megatron/transformer/deepseek_v4/test_p42_strided_v4_attention.py` |
| **G44a / G44b** | P42 | smoke + perf | EP=8 smoke + chrome trace with `PRIMUS_V4_ATTN_FUSED_PERMUTE=1`; trace `vec_elem<bf16_copy>` + `elem_unroll<copy>` buckets drop by ≥ 20 ms combined; iter time drops ≥ 10 ms vs P41 final. | `progress/p42/run_baseline_trace_ep8_p42.sh` + `develop/profile/profile-after-p42-ep8-<YYYYMMDD>.{md,html}` |
| **G45** | P43 | runtime (GPU) | Compressor APE FWD parity vs eager `reshape + mul + sum + add` within bf16 `atol=1e-3 rtol=1e-3` at fast + release tiers; BWD `gradcheck` fp32.  G42 (P39 router parity) carried green at all 3 score functions × {bias / no-bias} × {hash / topk}. | `tests/unit_tests/megatron/transformer/deepseek_v4/test_p43_compressor_ape_triton.py` |
| **G45a / G45b** | P43 | smoke + perf | EP=8 smoke + 50-iter, 3-run aggregate A/B with `PRIMUS_V4_ROUTER_TRITON=1` AND `PRIMUS_COMPRESSOR_APE_TRITON=1`; combined iter time drops ≥ 3 ms vs P42 final.  If 3-run mean is within ±1 ms band, router default stays `"0"`. | `progress/p43/run_baseline_trace_ep8_p43.sh` + `develop/profile/profile-after-p43-ep8-<YYYYMMDD>.{md,html}` |
| **G46** | P44 | runtime (GPU) | V4 attention FWD with fused sink epilogue parity vs eager `kernel + out * scale + sinks` within bf16 `atol=1e-3 rtol=1e-3`; BWD `gradcheck` covers `d_attn_sink` accumulator; release-tier slow.  Parametrise `attn_sink ∈ {present, absent}`. | `tests/unit_tests/megatron/transformer/deepseek_v4/test_p44_attn_sink_epilogue.py` |
| **G46a / G46b** | P44 | smoke + perf | EP=8 smoke + chrome trace; trace `vec_elem<mul_bf16>` AUnary bucket drops by ≥ 4 ms; iter time drops ≥ 2 ms vs P43 final. | `progress/p44/run_baseline_trace_ep8_p44.sh` + `develop/profile/profile-after-p44-ep8-<YYYYMMDD>.{md,html}` |
