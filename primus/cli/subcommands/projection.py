...
def run(args, overrides):
    ...
    if args.suite == "memory":
        # Benchmark / both modes need the Megatron backend on the import path
        # because they actually run the trainer.  Simulate stays import-free.
        memory_mode = getattr(args, "memory_mode", "simulate") or "simulate"
        ...
        # bench is skipped entirely, so we don't need to import the megatron
        # backend.  Same for pure-simulation mode.
...
def _add_memory_args(parser):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "  simulate  - Analytical only (default, no GPU required)\n"
            "  benchmark - Run sub-node layer benchmark, capture per-rank memory,\n"
            "              and analytically extrapolate to target cluster.\n"
            "              Intended for OOM-accurate cluster projections.\n"
