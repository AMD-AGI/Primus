...
__global__ void rs_acc_kernel(float* acc, const char* input_sym, size_t seg_off, size_t shard_elems,
    ...
    size_t c1 = c0 + chunk;
    if (c1 > shard_elems) c1 = shard_elems;
    if (c0 >= c1) return;
    size_t n = c1 - c0;
...
__global__ void gather_kernel_ctx(char* target, const char* src, size_t nbytes, const int* peers,
    ...
    int p = blockIdx.x;
    if (p < n_peers) {
        rocshmem_ctx_getmem_wg(ctxs[p % nctx], target + (size_t)peers[p] * stride_bytes, (void*)src,
                               nbytes, peers[p]);
    }
}
...
int gda_hdp_init() {
    ...
    hsa_status_t s =
        hsa_agent_get_info(g_gpu_agents[idx], (hsa_agent_info_t)HSA_AMD_AGENT_INFO_HDP_FLUSH, &hdp);
...
int gda_gather(long long target, long long src, size_t nbytes, const int* peers_host, int n_peers,
    ...
    if (nqp > 1) {
        ...
        gather_kernel<<<dim3(n_peers), dim3(blk), 0, 0>>>((char*)target, (const char*)src, nbytes,
                                                          d_peers, n_peers, stride_bytes);
    }
...
int gda_reduce_scatter_acc(long long acc_fp32, long long input_sym, size_t seg_off_bytes,
    ...
    if (dtype_code == DT_BF16) {
        ...
        rs_acc_kernel<float><<<grid, block>>>((float*)acc_fp32, (const char*)input_sym,
                                              seg_off_bytes, shard_elems, n_pes, (char*)scratch_sym,
