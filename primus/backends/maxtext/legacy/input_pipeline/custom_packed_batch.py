...
class _PackedBatch:
    ...
    def __init__(
        ...
        self._batch = tree_lib.map_structure(make_packed_buffer, length_struct, element_for_shapes)

        def make_packed_aux_info(length: int):
            return np.zeros(shape=(batch_size, length), dtype=np.int32)

        self._segmentations = tree_lib.map_structure(make_packed_aux_info, length_struct)
        self._positions = tree_lib.map_structure(make_packed_aux_info, length_struct)
    ...
    def _can_add_at_row(
        ...
        if any(tree_lib.flatten(length_exceeded)):
            raise ValueError("Inputs to PackAndBatchOperation must be truncated to max length.")
        ...
        is_row_free_struct = tree_lib.map_structure(
            _feature_will_fit, element_feature_lengths, self._first_free_cell_per_row, self._length_struct
        )

        ## Pick first row (if exists) where element can be added.
        for i in range(self._batch_size):
            if self._num_examples_per_row[i] < self._max_segments:
                row_is_free_per_feature = [free[i] for free in tree_lib.flatten(is_row_free_struct)]
    ...
    def add_element_to_batch(
        ...
        for per_feature_data in zip(
            ...
            value, batch_value, segmentations, positions, first_free_cell_per_row = per_feature_data
...
class CustomPackAndBatchOperation(Generic[_T]):
    ...
    def __post_init__(self):
        logging.error(
            "PackAndBatchOperation is deprecated. Please use" " lazy_dataset.FirstFitPackIterDataset instead."
        )
