...
from .transformer_modules.activation_layers import get_activation_layer
from .transformer_modules.norm_layers import get_norm_layer
from .transformer_modules.embed_layers import TimestepEmbedder, PatchEmbed, TextProjection, VisionProjection
from .transformer_modules.attention import parallel_attention
from .transformer_modules.posemb_layers import apply_rotary_emb, get_nd_rotary_pos_embed
from .transformer_modules.mlp_layers import MLP, MLPEmbedder, FinalLayer, LinearWarpforSingle
from .transformer_modules.modulate_layers import ModulateDiT, modulate, apply_gate
from .transformer_modules.token_refiner import SingleTokenRefiner

from .runtime_helpers import torch_compile_wrapper
from .byt5_mapper import ByT5Mapper

from .camera_rope import prope_qkv
from .config import HunyuanVideoConfig
...
class ARHunyuanVideo_1_5_DiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        # FSDP2's cast_forward_inputs would normally downcast these to the
        # parameter dtype on the way into each block, but it is skipped while
        # activation checkpointing recomputes a block. Cast here so the forward
        # and the recomputation see identical inputs.
        freqs_cos = freqs_cos.to(img.dtype) if freqs_cos is not None else None
        freqs_sin = freqs_sin.to(img.dtype) if freqs_sin is not None else None
        viewmats = viewmats.to(img.dtype) if viewmats is not None else None
        Ks = Ks.to(img.dtype) if Ks is not None else None
        ...
        for index, block in enumerate(self.double_blocks):
            ...
            if self.training and getattr(self, "gradient_checkpointing", False):
                ...
                def create_custom_forward(module, kwargs):
                    def custom_forward(img_in, txt_in):
                        return module(img=img_in, txt=txt_in, **kwargs)
                ...
                img, txt = block(img=img, txt=txt, **block_kwargs)
