...
WHY:
  ...
  each one during backward. That is ZeRO-3 traffic (~3P bytes/step) where ZeRO-2
  (~2P) was asked for. The ``reshard_after_forward=False`` on the root
  ``fully_shard()`` in our strategies only covers the root unit -- embeddings, norms,
  adaLN heads -- not the blocks, which hold nearly all the parameters.

  This affects **every** diffusion model on this path, not just ours, and is worth
  reporting upstream.
...
WHAT:
  ...
  **repairs rather than invents**: the value comes from ``manager_args``, which came
  from YAML. Installed unconditionally with no env gate, because it is
  behaviour-neutral when the YAML omits the key (``None`` stays ``None`` and today's
  heuristic applies), so there is nothing to gate.

  Note this is only half the fix. The Primus parallelization strategies must also
  forward the value into ``apply_fsdp2_sharding_recursively``; see
  ``models/{flux,ideogram4}/parallelize.py``. **Fixing either half alone changes
  nothing.**
...
def install() -> bool:
    ...
    # Imported inside install(), like the other hooks, so importing this module does
    # not drag in Automodel.
    ...
    if target is None or not callable(target):
        ...
        raise RuntimeError(
            ...
            "layout changed and the ZeRO-2 reshard patch cannot apply."
        )
