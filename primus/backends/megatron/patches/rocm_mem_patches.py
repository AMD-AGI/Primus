...
def patch_training_log_for_rocm_memory(ctx: PatchContext):
    ...
    This patch temporarily replaces print_rank_last during training_log execution,
    similar to how other patches replace dependencies (e.g., torch.cuda.stream).

    Strategy:
        1. Check if ROCm monitoring is enabled at patch time
        2. If disabled, skip patching entirely (zero overhead)
        3. If enabled, wrap training_log to temporarily replace print_rank_last
        4. Replacement only active during training_log call (scoped)

    Key insight:
        Instead of permanently replacing print_rank_last, we temporarily replace
        it ONLY when original_training_log is called. This ensures other functions
        (like validate) are never affected.
    ...
    try:
        ...
        def patched_training_log(
            ...
            Temporarily replaces print_rank_last during original_training_log call,
            similar to how build_model patch replaces torch.cuda.stream.
            """

            def enhanced_print_rank_last(log_string):
                """
                Enhanced print_rank_last that injects ROCm memory stats.

                This function only exists during original_training_log execution.
                """
            ...
            # Temporarily replace print_rank_last (similar to torch.cuda.stream pattern)
            import megatron.training.training as training_module

            _orig_print_rank_last = training_module.print_rank_last
            training_module.print_rank_last = enhanced_print_rank_last

            try:
                # Call original training_log with replaced print_rank_last
