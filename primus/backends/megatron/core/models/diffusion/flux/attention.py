...
class JointSelfAttention(Attention):
    ...
    def forward(
        ...
        # The Q/K RMSNorm (for_qk) and RoPE can promote Q/K to fp32 while V stays
        # bf16. AITER FlashAttention requires Q/K/V to share one dtype, otherwise
        # it raises "FlashAttention only support fp16, bf16 and fp8_e4m3". Align
        # Q/K to value.dtype at the kernel boundary; no-op when already equal.
