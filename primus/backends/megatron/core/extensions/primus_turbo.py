...
class PrimusTurboGroupedMLP(GroupedMLP):
    ...
    def forward(
        ...
        if self.use_turbo_groupmlp_act:
            activation_func_with_probs = partial(
                self.activation_func_with_probs, tokens_per_expert=tokens_per_expert
            )
        else:
            activation_func_with_probs = self.activation_func_with_probs
        ...
        if permuted_local_hidden_states.nelement() != 0:
            ...
            if self.activation_recompute:
                intermediate_parallel = self.activation_checkpoint.checkpoint(
                    activation_func_with_probs, fc1_output, permuted_probs.unsqueeze(-1)
                )
                fc2_output = self.grouped_gemm(intermediate_parallel, w2, tokens_per_expert, trans_b=False)
                self.activation_checkpoint.discard_output_and_register_recompute(fc2_output)
            else:
                intermediate_parallel = activation_func_with_probs(fc1_output, permuted_probs.unsqueeze(-1))
                fc2_output = self.grouped_gemm(intermediate_parallel, w2, tokens_per_expert, trans_b=False)
        else:
