...
WP9 found that FlyDSL JIT was broken image-wide: the ``ld.lld`` in the
...
a future toolchain fix has exactly one place to land. ``DECISIONS.md`` (WP9)
records both the defect and the two ways of "fixing" it that do not work: a
symlink (preserves ``argv[0]``) and prepending to ``PATH`` (MLIR looks inside
the ROCm *toolkit* directory, not on ``PATH``).
"""
