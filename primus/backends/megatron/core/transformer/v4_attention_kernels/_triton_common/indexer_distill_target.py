...
is supposed to imitate. A CSA layer takes one softmax jointly over the sliding
window, the sparse compressed entries and the sink, so the share of a head's
attention that lands on the compressed branch is
``exp(compressed_lse - full_lse)`` -- below 1, and different for every head.
Normalising each head over the compressed entries alone throws that away and
head-sums as if every head cared about the compressed branch equally, which
over-weights the heads that are almost entirely local. Passing the log mass of
the non-compressed part (window + sink) puts it back in the denominator.
