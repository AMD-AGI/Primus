...
class DeprecatedMoETokenDispatcher:
    """
    ...
    @abstractmethod
    def token_permutation(self, tokens: torch.Tensor, probs: torch.Tensor, routing_map: torch.Tensor):
...
class DeprecatedMoEAllGatherTokenDispatcher(DeprecatedMoETokenDispatcher):
    """
    ...
    def token_permutation(self, hidden_states: torch.Tensor, probs: torch.Tensor, routing_map: torch.Tensor):
        ...
        if self.tp_size > 1 or self.ep_size > 1:
            ...
            with torch.no_grad():
                ...
                routing_map = gather_from_sequence_parallel_region(routing_map, group=self.tp_ep_group)
...
class DeprecatedMoEAlltoAllTokenDispatcher(DeprecatedMoETokenDispatcher):
    """
