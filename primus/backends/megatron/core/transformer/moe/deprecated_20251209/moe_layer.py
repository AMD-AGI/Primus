...
from primus.backends.megatron.core.transformer.moe.deprecated_20251209.token_dispatcher import (
    DeprecatedMoEAllGatherTokenDispatcher,
    DeprecatedMoEAlltoAllTokenDispatcher,
)


@dataclass
class DeprecatedMoESubmodules:
...
class DeprecatedBaseMoELayer(MegatronModule, ABC):
    ...
    def __init__(
        ...
        super(DeprecatedBaseMoELayer, self).__init__(config)
        self.config = config
...
class DeprecatedMoELayer(DeprecatedBaseMoELayer):
    """Mixture of experts Layer **currently only supports no token dropping**.

    Args:
        DeprecatedBaseMoELayer (MegatronModule): Base class for MoE layers
    """
    ...
    def __init__(
        ...
        super(DeprecatedMoELayer, self).__init__(config=config, layer_number=layer_number)
        ...
        if config.moe_token_dispatcher_type == "allgather":
            self.token_dispatcher = DeprecatedMoEAllGatherTokenDispatcher(
                self.num_local_experts, self.local_expert_indices, config=self.config
            )
        elif config.moe_token_dispatcher_type == "alltoall":
            self.token_dispatcher = DeprecatedMoEAlltoAllTokenDispatcher(
                self.num_local_experts, self.local_expert_indices, config=self.config
            )
