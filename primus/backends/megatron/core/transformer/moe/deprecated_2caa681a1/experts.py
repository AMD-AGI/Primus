...
class DeprecatedGroupedMLP(MegatronModule):
    """An deprecated implementation of the Experts layer using GroupedGEMM.
    ...
    def __init__(
        ...
        assert config.moe_latent_size is None, "MoE latent projection not supported in GroupedMLP yet."
    ...
    def sharded_state_dict(self, prefix="", sharded_offsets=(), metadata=None):
        ...
        def _break_into_individual_experts(
            ...
            for local_expert_idx, expert_ten in enumerate(experts_ten):
                ...
                expert_key = key.replace(f"{prefix}experts.", f"{prefix}experts.{global_expert_idx}.")
        ...
        for expert_local_idx in range(self.num_local_experts):
            ...
            for mod in ["linear_fc1", "linear_fc2"]:
                ...
                sharded_state_dict[f"{prefix}expert{expert_global_idx}.{mod}._extra_state"] = (
                    make_sharded_object_for_checkpoint(None, expert_key, expert_sharded_offsets, replica_id)
                )
