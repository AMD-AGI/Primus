...
#
# This file is ported from NVIDIA Megatron-LM PR #2690 (Paged Stashing) so that
# Primus can enable the feature on top of stock Megatron-LM without forking the
# third_party submodule. The content is a copy of
# ``megatron/core/transformer/moe/paged_stash.py`` from that PR, and is installed
# at runtime into ``megatron.core.transformer.moe.paged_stash`` by
# ``primus.backends.megatron.patches.moe_patches.paged_stash_patches``.
#
# Upstream reference:
#   https://github.com/NVIDIA/Megatron-LM/pull/2690
