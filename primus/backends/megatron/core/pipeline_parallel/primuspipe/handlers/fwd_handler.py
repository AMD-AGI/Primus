...
from megatron.core.pipeline_parallel.schedules import deallocate_output_tensor, forward_step
...
def megatron_fwd_handler(node: SchedulerNode, idx: int, scheduler_table: list[SchedulerNode]):
    ...
    if get_args().dump_pp_data:
        forward_step_func = fwd_bwd_wrapper(
            forward_step, "fwd", minibatch=node.mini_batch, chunk=node.chunk
        )
    
    is_last_stage = node.meta["last_pp_stage_rank"] == node.meta["pp_rank"] and node.chunk == node.meta["vpp_size"] - 1
