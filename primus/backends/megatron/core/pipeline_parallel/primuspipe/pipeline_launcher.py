...
import torch
from primus.backends.megatron.core.pipeline_parallel.zerobubble.runtime import bootstrap_and_profile_p2p_communication
...
from megatron.core.pipeline_parallel.schedules import clear_embedding_activation_buffer, finish_embedding_wgrad_compute, forward_step, get_tensor_shapes
from megatron.core.process_groups_config import ProcessGroupCollection
from megatron.core.utils import get_model_config, get_model_type
from primus.backends.megatron.core.pipeline_parallel.primuspipe.handlers import megatron_primuspipe_handler_dict
from primus.core.pipeline_parallel.scheduler.schedule_table_factory import produce_schedule_instance
...
class PrimusPipelineParallelLauncher:
    def __init__(self):
        ...
        assert self.pp_algorithm in ("1f1b", "1f1b-interleaved", "zero-bubble", "zbv-formatted", "zbv-greedy")

        if self.pp_algorithm in ("1f1b", "zero-bubble"):
            assert self.vpp_size == 1, "1f1b and zero-bubble require vpp_size to be 1"
        if self.pp_algorithm in ("zbv-formatted", "zbv-greedy"):
            assert self.vpp_size == 2, "zbv-formatted and zbv-greedy require vpp_size to be 2"

        if self.pp_algorithm == "zbv_greedy":
            assert args.cached_fwd_chunks_limit is not None and args.cached_fwd_chunks_limit > 0, "cached_fwd_chunks_limit must be set for zbv-greedy"
        ...
        *,
        ...
        if config.finalize_model_grads_func is not None and not forward_only:
            ...
            finish_embedding_wgrad_compute(
                config, embedding_module, self.schedule_instance.last_pp_stage_rank() == self.pp_rank, tp_group
            )
