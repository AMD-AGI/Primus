...
import megatron.core.jit as mc_jit

# See primus.diffusion.models.flux.flux_layer_spec: torch.compile via default jit_fuser
# deadlocks under torchrun + DataLoader workers (TorchInductor async compile).
mc_jit.disable_jit_fuser()
...
from primus.diffusion.models.common.dit_attention import (
    DiTCrossAttention,
...
class WanLayerWithAdaLN(TransformerLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Expand modulation tensors from (1, B, h) to (s, B, h) for shape consistency
        # (historically also worked around torch.compile shape issues when jit_fuser was enabled).
