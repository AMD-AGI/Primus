...
class TestMegatronArgBuilderFiltering:
    ...
    @patch("primus.backends.megatron.builders.argument_builder._load_megatron_defaults")
    ...
    @patch("primus.backends.megatron.builders.argument_builder._load_megatron_defaults")
    def test_none_values_ignored(self, mock_load_defaults):
        ...
        builder.update(
            {
                "num_layers": None,  # Should be ignored
                "hidden_size": 4096,  # Should be accepted
            }
        )

        assert "num_layers" not in builder.overrides
        assert "hidden_size" in builder.overrides
        assert builder.overrides["hidden_size"] == 4096
...
class TestMegatronArgBuilderOverrides:
    ...
    @patch("primus.core.runtime.distributed.get_distributed_info")
    @patch("primus.backends.megatron.builders.argument_builder._load_megatron_defaults")
