...
在我们的适配过程中，这条时间线由 `odc/fsdp/fsdp2.py` 明确编排：
...
因为推梯度是"发射后不管"，理论上它可以和后续的反向计算重叠；只在 minibatch 末尾做一次统一的等待/结算。**这是 ODC 省气泡的关键**——它把"逐层的通信等待"折叠成"minibatch 末的一次等待"。

**原版实现的通信底座**：节点内用 CUDA IPC（把 peer 的显存映射进本进程直接读写），节点间用 NVSHMEM（GPU-initiated 单边 RDMA）。移植到 AMD，就是要把这两层换成 ROCm 上的等价物：节点内走 **XGMI + HIP IPC**（peer 显存直读写），节点间走 ** rocSHMEM（默认，GPU-initiated RDMA）或 MORI**。
...
我们用 PyTorch Profiler 抓了单机/双机、NCCL/ODC、pad/nopad 的真实 trace。下面这组实测截图由浅及深地展示"ODC 到底改变了什么"：先看逐层集合屏障如何消失（图 1、图 2），再看变长微批相对补空桶的形态差异（图 3、图 4）。

### 图 1 —— 单机 1.5B，NCCL 基线：逐层同步屏障

![图1：单机1.5B，NCCL基线——逐层集合同步屏障](odc_blog/fig1_nccl_perlayer_barriers.png)

**图注**：单机 8 卡、DeepSeek-R1-Distill-Qwen-1.5B、标准 FSDP2 + RCCL。多个 rank（`[R0]/[R1]/[R2]`）的 `stream 14` 上密密麻麻地铺着 `nccl:reduce_scatter_base` + `ncclDevKernel_Generic_2`，**每一层一道隐式的对齐屏障**；`stream 0` 是计算（`void ck_tile`）。这些集合核就是"通信对齐屏障"——所有 rank 必须在此处对齐才能继续。
...
### 图 3 vs 图 4 —— odc_nopad vs odc_pad：变长微批 vs 补空桶

这两张图解释了 ODC 真正的杀手锏，也是 `nopad` 为什么只有 ODC 能跑。**图 3 是"没有补空桶"的 odc_nopad，图 4 是"有补空桶"的 odc_pad。**

两张 trace 都取自同一档配置：`global_batch_size=16`、`dp=8`，于是每个 DP rank 平均只分到 `16/8 = 2` 个样本；这些变长样本经 KK（Karmarkar-Karp）均衡跨 rank 分配后，在本 rank 内打包成 seq≈65536（64K token）的微批。**如图3所示**：每 rank 只有 2 个样本，意味着"样本长度的天然抖动"几乎无法在 rank 内被平均掉——一个 rank 可能两条都是长文（要拆成 2 个 64K 微批），另一个 rank 可能两条都短（1 个微批就装下）。于是"各 rank 微批数不一致"在这一档被放大到最明显，正好用来对比 pad / nopad 两条路线：**pad 强行把每个rank对齐成相同微批数（付出空桶算力），nopad 允许各跑各的（免掉空桶）。** 这也是 ODC 的价值最集中体现的一档（对应第 4 节里单机 gbs16 的峰值加速比）。

![图3：odc_nopad——各 rank 按真实变长负载跑、微批数可不同、免补空桶，trace 呈两段密集 burst](odc_blog/fig6_odc_nopad.png)

**图 3（odc_pad，LB-Micro `same_num_in_dp`）——为了兼容集合语义而付出的代价。** pad 路线要求**所有 rank 的微批数强制相同**：负载最重的那个 rank 决定了"每 rank 要跑几个微批"，其它较轻的 rank 就必须**补齐到同样的微批数**——补上的是**"空桶"（padding 微批）**，里面塞的是无意义的 padding token，纯粹为了让各 rank 步调对齐、能安全地逐微批调集合。反映到 trace 上，整体被压成**单个连续的密集 burst**：因为大家被对齐成同一节奏、没有错峰，表面上"更整齐"，但这份整齐是靠**空桶做无用功**换来的——那些 padding 微批照样吃前向/反向的算力和显存带宽，却不贡献任何有效梯度。

![图4：odc_pad——为对齐各 rank 微批数而补"空桶"，trace 呈单个密集 burst](odc_blog/fig5_odc_pad.png)

**图 4（odc_nopad，LB-Mini 变长微批数）——这才是 ODC 想要的形态。** 因为允许各 rank 微批数不同，数据层（LB-Mini）按每 rank 的**真实变长负载**排布：有的 rank 分到的两条样本合起来要 2 个 64K 微批，有的只要 1 个。反映到 trace 上，主线程（`thread ... python`）与 autograd 反向线程（`pt_autograd_0`）上呈现**两段清晰分离的密集 burst**——这不是"停顿"，而是不同 rank / 不同微批各自按自己的节奏推进：**快 rank 跑完自己的微批不必空等慢 rank，慢 rank 也不用被拉着走**。**关键在于：这种"各 rank 微批数可不等"的形态只有 ODC 的单边 p2p 能驱动。** 集合通信（all-gather / reduce-scatter）要求 DP 组里每个 rank 以**完全相同的次数**进入同一个原语——一旦某个 rank 少调一次（微批数不同就会发生），跨节点集合就会 barrier 次数不匹配、直接 hang 死。ODC 的 `getmem`/`putmem` 是单边的，谁需要谁发起、发射后不管，天生不受"调用次数必须一致"的约束，这才让 nopad 成为可能（实现细节见 `odc/primitives/scatter_accumulate.py`）。

**两图对读，结论很直接：** odc_nopad（图 4）和 odc_pad（图 3）的**有效计算量完全相同**，差别只在 pad 多做了一批空桶。nopad 把这批空桶省掉——省下的正比于"各 rank 负载的不均衡程度"，在 gbs 小（每 rank 样本少、抖动大）、序列变长明显的场景里最可观。而"变长微批数、免补空桶"这条路，**只有 ODC 的单边 p2p 走得通**：集合基线为了不死锁只能 pad，从而把 padding 的算力浪费当成必付的税。这就是 ODC 在"负载不均衡"场景下的结构性红利，也是后面第 4 节 gbs16 峰值加速比里"变长负载均衡"那一块收益的来源。
...
单机的加速比曲线呈**"先升后回落"的驼峰形**：

- **gbs8 略慢（~0.95×）——固定开销摊不掉。** 单机每步的通信总量本来就小（1.5B 模型、节点内 XGMI），但 ODC 每个 minibatch 仍要付一笔**近乎固定的开销**：per-minibatch 的 `barrier` + 结算（scatter-accumulate 的 `sync`）+ 握手。gbs 只有 8 时，一步里真正的前反向计算量太小，这笔固定开销的**占比被放大**；再加上此时反向的 p2p 推送还没能和计算 overlap（见第 5 节图 5），固定成本"露"在关键路径上收不回来，于是净体验比高度优化的 RCCL collective 略慢。这不是 ODC 的败笔，而是"红利需要足够的 batch 来兑现"的直接体现。

- **gbs16 见峰（~1.20×）——两块优势。** **两个独立的收益源在这里同时最大化**：
  ...
  2. **负载侧：变长均衡免补空桶。** gbs16 / dp8 → 每 rank 只有 2 个样本，各 rank 负载抖动最大（见第 3 节图 3/图 4），nopad 允许各 rank 微批数不等、**免去 pad 的空桶算力浪费**，这块红利在小 gbs、强变长时最高。

  把这两块拆开看得最干净——用同节点 apples-to-apples 的 `NCCL_pad → ODC_pad → ODC_nopad` 阶梯（另一组 50 轮实验、只逐步改一个变量）：`NCCL_pad → ODC_pad` 先快 ~**9.8%**（**纯减小每层同步的红利**：数据、pad 策略完全一样，只把 RCCL 的逐层集合换成 ODC 的 XGMI 按需 p2p，每 minibatch 同步一次省下逐层同步）；`ODC_pad → ODC_nopad` 再快 ~**10.2%**（**纯负载均衡红利**：通信后端一样，只把"补空桶"换成"变长微批"）。两者复合起来 `1.098 × 1.102 ≈ 1.21×`（约合 ~20%），正好落在实测的 gbs16 峰值上。**即单机的收益 = XGMI 按需通信 + 变长负载均衡，两块可加、缺一不可。**

- **大 gbs 回落到持平（gbs64→1.0×、gbs128→~0.99×）——compute 主导、红利被摊薄。** gbs 越大，一步里的前反向 GEMM 计算量越大、占满了 GPU；而上面那两块红利是**每步近乎固定或按比例但增速慢于 compute** 的量。当 compute 成为绝对大头，固定红利被摊薄到噪声级别，ODC 与 RCCL 自然收敛到持平——这也解释了为什么单机是"驼峰"而非"单调"。
...
| 32 | ≈ 0.92×（nopad）/ 0.87×（pad） | 和 RCCL 性能近似 |
| 64 | ≈ **1.13×（odc_nopad 反超）** | 大 batch 摊薄跨节点固定开销 + 变长气泡收益兑现 |
| 128 | ≈ **1.16×** | gbs 越大、跨节点集合越贵，ODC 的摊销收益越大 |
...
- **gbs64 起 odc_nopad 反超（~1.13×）、gbs128 达 ~1.16×——大 batch 兑现结构性红利。** 到 gbs64，摊薄后的固定开销已低于"省集合 + 免空桶气泡"的收益，odc_nopad **首次反超 RCCL**；gbs128 时 batch 更大、跨节点集合（RCCL 这边）越贵，而 ODC 的按需 p2p + 变长均衡越划算，加速比进一步升到 ~**1.16×**。这条"设备越多/越跨节点/负载越不均衡，ODC 越赚"的方向，与论文 Fig.10 的结论**完全一致**——ODC 的价值本就随规模和异质性增长。
...
1. **移植跑通**：ODC 的算法层（gather / scatter-accumulate、per-minibatch 同步、LB-Mini 变长均衡）已在 AMD ROCm / MI300X 上用 **MORI（默认）/ rocSHMEM** 后端跑通，**单机 + 双机**都能正确收敛（loss 与单机基线对齐、0 nan）。
2. **收益场景清晰**：ODC 在**"大 batch / 跨节点 / 有负载不均衡"**时收益最大——单机 gbs16 峰值 ~1.20×，双机 gbs64 起反超 RCCL、gbs128 达 ~1.16×。这与论文"ODC 从根本上减少 FSDP 的负载不均衡气泡、且加速比随设备数增长"的结论一致。

### 5.1 进一步优化的方向：settle 还没和反向 overlap

：**ODC 的结算（settle）目前基本"露"在反向的关键路径上，没能和计算重叠。** 这一点在 trace 上看得非常清楚，也正好和成熟的原生 NCCL 形成刺眼的对照——下面两张图（图 5 vs 图 6）。

![图5：开启ODC 反向——FSDP::post_backward_reduce 堆叠聚集在同一时刻，未与计算铺开重叠](odc_blog/fig4_odc_settle_no_overlap.png)

**图 5（ODC 反向：settle 堆在关键路径上，没 overlap）**：放大 ODC 的反向（`ProfilerStep#8`）。`stream 11` 上是 `FSDP::all_gather`，而 `stream 16–23` 上一整列 `FSDP::post_backward_reduce`（即 scatter-accumulate 的结算 / `WAIT_ACC` 等待，带 `M/n` 标记）**堆叠聚集在同一时刻**、并未铺开去和 `stream 0` 的计算重叠——结算被串在了关键路径上，overlap 很低（实测 ~5%）。也就是说，虽然我们已经把跨 rank 同步从 per-iteration 降到 per-minibatch（第 3 节图 2），但**每层结算的等待仍然"露"在计算之外**，没有被后续反向计算掩盖。这也解释了前面 4.1 里 gbs8、4.2 里小 gbs 为什么略慢——固定的结算/同步开销收不回来。

![图6：原生 NCCL 反向——reduce_scatter_base 与 void ck_tile 计算在同一时段并行重叠（红框，ProfilerStep#10）](odc_blog/fig3_nccl_backward_overlap.png)

**图 6（对照：原生 NCCL 反向能把通信藏进计算）**：作为标杆，看原生 FSDP2 + NCCL 的反向。红框圈出的是同一时段内 `stream 14` 的 `nccl:reduce_scatter_base`（+`ncclDevKernel_Generic_2`）与 `stream 0` 的 `void ck_tile` 计算**并行重叠**（`ProfilerStep#10`）——`reduce_scatter` 的通信核被 FSDP2 的 **prefetch** 机制**藏在了计算后面**：下一层的通信在当前层计算时就已经发起，通信/计算实打实地并行。这正是成熟 RCCL/NCCL + FSDP2 多年打磨出来的 overlap 能力。


**展望（待优化点，按性价比）**：

- **让 settle 与反向 overlap **：这就是图 5 那部分。原生 NCCL 靠 prefetch 把通信藏进计算，我们要做的是一条**跨迭代的软流水**——用独立 stream + event，把"上一组 microbatch 的 reduce-scatter / settle"与"本组 microbatch 的反向计算"重叠起来，只在 minibatch 末做一次总的 join。此种方式最可能把双机中等 gbs 追平、乃至把峰值再往上顶的方向。
