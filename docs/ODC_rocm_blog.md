...
本节把前文实验的完整复现流程沉淀成可照做的教程。软件基线是两个**配套**的 PR，缺一不可：

- **Primus-Turbo [#409](https://github.com/AMD-AGI/Primus-Turbo/pull/409)**：把 ODC 的 rocSHMEM 单机（host / XGMI-IPC）与多机（GDA，GPU-Direct RDMA）后端并入算子库，暴露为 `primus_turbo.pytorch._C.odc_rocshmem_host` / `odc_rocshmem_gda` 两个 pybind 子模块。
- **Primus [#864](https://github.com/AMD-AGI/Primus/pull/864)**：ODC 成为 `primus/core/odc/` 下的**纯 Python** in-tree 模块，rocSHMEM 算子改为从 Primus-Turbo 消费（不再在树内编 ctypes `.so`，`build_rocshmem_backend.sh` 已删除）。

分工上：**Primus 负责算法层**（gather / scatter-accumulate、LB-Mini 变长均衡、per-minibatch 结算），**Primus-Turbo 负责通信算子**（rocSHMEM host/GDA）。
...
- ROCm 7.2（gfx942）、Docker、（可选）SLURM。
- 一个装好 ROCm / PyTorch / Megatron 依赖的镜像（本文用内部镜像；读者用等价的 Primus 官方镜像即可）。
...
> **建议单机 / 双机各编一份 Turbo。** 单机走 host/XGMI-IPC（`odc_rocshmem_host`），双机走 GDA（`odc_rocshmem_gda`）；两者依赖的 rocSHMEM 连接后端不同，单机 IPC 的对称堆与双机 GDA 混用时容易在 symmetric-heap 上打架。我们的做法是保留两个目录——`Primus-Turbo`（`ROCSHMEM_HOME=rocshmem_gda`，多机，带 lto=1）与 `Primus-Turbo-single`（`ROCSHMEM_HOME=rocshmem_single`，单机 IPC），运行时用 `PRIMUS_TURBO_PATH` 分别指向。
...
- **第 1 参数**选 ODC 的 P2P 后端：`mori`（MORI-SHMEM）或 `rocshmem`（本文移植的 rocSHMEM 单边 p2p 后端）。`run_odc.sh` 默认 `export ODC_ENABLE=1`，因此基线臂要显式追加 `ODC_ENABLE=0` 才回退到标准 FSDP2 + RCCL（见下表）。
...
三条对照臂由此拼出（与第 4 节表格严格对应）：

| 臂 | `run_odc.sh` 命令要点 |
|---|---|
| `NCCL_pad`（基线） | `mori     pad   <yaml> <name> ODC_ENABLE=0 LB_MINI_FORCE_DATA=1` |
| `odc_pad` | `rocshmem pad   <yaml> <name>` |
| `odc_nopad` | `rocshmem nopad <yaml> <name>` |
...
> - 复现本文数字请把 `train_iters` 设为 **单机 200 / 双机 100**（与 6.5 / 6.6 一致）；加速比是 `ms/iter` 之比，与 iter 数无关，只是 iter 越多、`ms/iter` 中位数越稳。
...
# 基线：标准 FSDP2 + RCCL
bash "$RUN" mori     pad   "$EXP" nccl_pad  ODC_ENABLE=0 LB_MINI_FORCE_DATA=1
# ODC：pad / nopad
bash "$RUN" rocshmem pad   "$EXP" odc_pad
bash "$RUN" rocshmem nopad "$EXP" odc_nopad
```

单机走节点内 XGMI + HIP-IPC，reduce-scatter-accumulate 是 device 侧的 owner-pull（on-chip fp32 求和、无 host watcher 子进程）。本文单机各档跑 **200 iter**（实验驱动 `single_driver.sh` 里设 `ITERS=200`）。

### 6.6 双机 14B（16 卡，GDA）

双机在**每个节点各起一个 rank**（NODE_RANK 0/1），rocSHMEM 用 **uid-over-socket** bootstrap（PR #864 后不再用 MPI，纯 torchrun 起）。关键是一组 GDA/RoCE 环境变量：

```bash
# 两个节点容器内都执行；NODE_RANK 分别取 0 / 1，MASTER_ADDR 指向 node0
export PRIMUS_TURBO_PATH=/workspace/code/Primus-Turbo          # 多机 GDA 版(lto=1)
export NNODES=2 GPUS_PER_NODE=8 NODE_RANK=<0|1>
export MASTER_ADDR=<node0-host> MASTER_PORT=29642
export HF_HOME=/workspace/hf_cache HF_HUB_OFFLINE=1 PRIMUS_SKIP_PIP=1

# --- GDA / RoCE 部署环境（按集群 NIC 调整 HCA_LIST）---
GDA_KV="ODC_ROCSHMEM_GDA=1 ROCSHMEM_GDA_PROVIDER=mlx5 \
  ROCSHMEM_BOOTSTRAP_SOCKET_IFNAME=eth0 ROCSHMEM_HEAP_SIZE=8589934592 \
  ROCSHMEM_HCA_LIST=mlx5_0,mlx5_2,mlx5_3,mlx5_4,mlx5_5,mlx5_7,mlx5_8,mlx5_9 \
...
# 基线：跨节点 RCCL
bash "$RUN" mori     pad   "$EXP" dual_nccl_pad \
  NCCL_SOCKET_IFNAME=eth0 GLOO_SOCKET_IFNAME=eth0 NCCL_IB_DISABLE=0 NCCL_IB_GID_INDEX=3 ODC_ENABLE=0 LB_MINI_FORCE_DATA=1
# ODC：pad / nopad（GDA）
bash "$RUN" rocshmem pad   "$EXP" dual_odc_pad   $GDA_KV
bash "$RUN" rocshmem nopad "$EXP" dual_odc_nopad $GDA_KV
```
...
- 本文双机各档跑 **100 iter**（实验驱动 `run_pair.sh` 默认 `ITERS=100`）。取稳定段中位数算 `ms/iter`。

### 6.7 读结果、算加速比

每个 run 的训练日志会打印 `iteration N/M | ... | elapsed time per iteration (ms): X`。取稳定段（跳过前几步 warmup）的 `ms/iter` 中位数，按

> 加速比 = `NCCL_pad 的 ms/iter` ÷ `本 run 的 ms/iter`（>1 即比 RCCL 快）

即可复现第 4 节的表格。同时核对三条臂的 `lm loss` 收敛曲线是否与基线对齐、全程 0 nan——这是"跑对了"的判据。
