...
对照上图：**gather** 是"缺哪片就去哪台拉哪片"，**scatter-accumulate** 是"算完哪片就把它推给对应 owner 去累加"。两者都由需要方单边发起、发射后不必等对端进入同一次集合——这正是 ODC 能容忍"各 rank 步调不一致"的根本原因。官方仓库也点明：底座在节点内用 CUDA IPC、节点间用 NVSHMEM 实现这套 RDMA 单边原语（第 4 节我们会把它换成 ROCm 上的等价物）。
...
光讲原理不够，我们用 PyTorch Profiler 抓了单机/双机、NCCL/ODC、pad/nopad 的真实 trace。下面六张图（均为实测截图）由浅及深地展示"ODC 到底改变了什么"、以及"当前还差在哪"。
...
**图注**：放大 ODC 的反向（`ProfilerStep#8`）。`stream 11` 上是 `FSDP::all_gather`，而 `stream 16–23` 上一整列 `FSDP::post_backward_reduce`（即 scatter-accumulate 的结算 / `WAIT_ACC` 等待，带 `M/n` 标记）**堆叠聚集在同一时刻**、并未铺开去和 `stream 0` 的计算重叠——结算被串在了关键路径上，overlap 很低（实测 ~5%）。也就是说，虽然我们把同步降到了 per-minibatch，但每层的结算等待仍然"露"在计算之外。
...
**图注**：作为对照，看原生 FSDP2 + NCCL 的反向。红框圈出的是同一时段内 `stream 14` 的 `nccl:reduce_scatter_base`（+`ncclDevKernel_Generic_2`）与 `stream 0` 的 `void ck_tile` 计算**并行重叠**（`ProfilerStep#10`）——`reduce_scatter` 的通信核被 FSDP2 的 prefetch 机制**藏在了计算后面**（下一层的通信在当前层计算时就已发起），有实打实的 comm/compute overlap。

图 3 vs 图 4 放一起看，结论很清楚：**在"通信能否被计算掩盖"这件事上**，图 4 里成熟的 RCCL/NCCL + FSDP2 prefetch 把通信藏进了计算，而图 3 里 ODC 的 post-backward 结算却堆在关键路径上没铺开——目前 RCCL 在这一点上领先于我们的 ODC 移植。这不是 ODC 思想的问题，而是我们移植版本的结算流水还没打磨到位——是明确的 roadmap，而非死结。

### 图 5 vs 图 6 —— odc_pad vs odc_nopad：补空桶 vs 变长微批

这两张图解释了 ODC 真正的杀手锏，也是 `nopad` 为什么只有 ODC 能跑。

设定：`global_batch_size=16`、`dp=8`，于是 `16/8 = 2` 个样本/rank，KK（Karmarkar-Karp）均衡后打包成 seq≈65536 的微批。

![图5：odc_pad——为对齐各 rank 微批数而补"空桶"，trace 呈单个密集 burst](odc_blog/fig5_odc_pad.png)

**图 5（odc_pad，LB-Micro `same_num_in_dp`）**：为了让所有 rank 微批数相同，负载重的 rank（样本更长、超过单微批 token 上限）会拆出更多微批，其它 rank 就得**补齐到相同微批数**——补的是"空桶"（padding 微批）。trace 上呈**单个密集 burst**，其中夹着这些做无用功的空桶微批。

![图6：odc_nopad——各 rank 按真实变长负载跑、微批数可不同、免补空桶，trace 呈两段密集 burst](odc_blog/fig6_odc_nopad.png)

**图 6（odc_nopad，LB-Mini 变长微批数）**：由于各 rank 拿到的样本长度不同，**各 rank 的微批数可以不一样**（有的 rank 1 个微批，有的 2 个），trace 上呈**两段密集 burst**，各 rank 按自己的真实负载往前跑、谁也不等谁、免去补空桶——**这只有 ODC 的单边 p2p 能驱动**：集合通信要求各 rank 调用次数一致，微批数不等会直接死锁。

**odc_nopad 的优势 = 免掉补空桶**（图 6 vs 图 5）。而"变长微批数"这件事本身，**只有 ODC 的 p2p 能做**（`odc/primitives/scatter_accumulate.py` 的 DEFER 注释里写得很清楚：nopad 下各 rank 微批数不同，逐微批调集合会 barrier 次数不匹配 → 跨节点死锁）。这也是为什么在集合基线里，要跑变长数据就必须 pad（`LB_MINI_SAME_MICRO=1`），从而付出 padding 的算力浪费。
...
## 5. 实验结果数据说话：单机 1.5B 与双机 14B 的加速比
...
### 5.1 单机 1.5B（8 GPU，500 轮，总时间口径）

| gbs | ODC_nopad 加速比 | 趋势解读 |
|---|---|---|
| 8 | ≈ **0.95×**（略慢） | minibatch 太小，反向过程中p2p 的固定开销无法overlap起来，反而略慢于 RCCL |
...
拆解单机为什么会快（同节点 apples-to-apples 的 `NCCL_pad → ODC_pad → ODC_nopad` 阶梯，另一组 50 轮实验）：`NCCL_pad` → `ODC_pad` 先快 ~**9.8%**（按需通信，每minibatch的同步开销小于每层的同步开销），再到 `ODC_nopad` 又快 ~**10.2%**（免掉补空桶的负载均衡红利），合计 ~**20%**。即单机的收益 = **XGMI 按需通信 + 变长负载均衡**两块叠加。

### 5.2 双机 14B（16 GPU，100 轮；另有干净仓库 50 轮交叉印证，总时间口径）

| gbs | ODC 加速比 | 趋势解读 |
|---|---|---|
| 16 | ≈ **0.78–0.83×**（明显慢） | 因为缺少GDRW的实现，只能手动实现同步读这个动作，保证 RDMA 写有序且对远端是正确可见，每层反向多了一个同步的开销|
| 32 | ≈ 0.92×（nopad）/ 0.87×（pad） | 收窄，但仍不及 RCCL |
| 64 | ≈ **1.16×（odc_nopad 反超）** | 大 batch 摊薄了跨节点异步规约开销 |
...
lr 对齐 qwen14B 续训设置（`2e-6`）。两处曲线几乎重合，说明**移植行为一致、无功能回归**。
