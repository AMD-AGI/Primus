...
- **① Primus-Turbo——通信本体。** 把 ODC 的 rocSHMEM 单机（host / XGMI-IPC）与多机（GDA，GPU-Direct RDMA）后端并入算子库，暴露为 `primus_turbo.pytorch._C.odc_rocshmem_host` / `odc_rocshmem_gda` 两个 pybind 子模块。**缺了它 ODC 根本无法做 P2P 通信**。这套算子的**源码与 pybind wiring 确已并入 [Primus-Turbo](https://github.com/AMD-AGI/Primus-Turbo) 官方 `main`**（PR #409，`csrc/kernels/odc_rocshmem/`，且 `setup.py` 已内置 GDA 必需的 `--lto-partitions=1`）。**但注意：直接 `pip install "git+…@main"` 并装不出算子**——这两个 pybind 子模块被 `#ifndef DISABLE_ROCSHMEM` 门控，`setup.py` 构建时用 `find_rocshmem_library()` 探测 rocSHMEM：只有设了 `ROCSHMEM_HOME`（+`MPI_HOME`）指向一份**预编好的 rocSHMEM 静态库**才会真正编入，否则就加 `-DDISABLE_ROCSHMEM` 把算子**编译掉** → `has_host=False has_gda=False`（我们在镜像内 pip 装 `@main` 实测即如此）。rocSHMEM 是**非 pip 分发的外部依赖**，所以正确做法是**从源码构建 Primus-Turbo、把 `ROCSHMEM_HOME` 指向预编 rocSHMEM**（单机 host 与双机 GDA 各需对应的一份），再经 `PRIMUS_TURBO_PATH` + `.image_bak` 消费（见 6.3）。
...
> **为什么不能只 `pip install @main`（本节最重要的一条）。** ODC 的 rocSHMEM host/GDA 算子源码与 pybind wiring **确实在 Primus-Turbo 官方 `main`**（PR #409）。但这两个子模块被 `#ifndef DISABLE_ROCSHMEM` 门控：`setup.py` 构建时调 `find_rocshmem_library()` 探测 rocSHMEM，**只有** `ROCSHMEM_HOME`（+`MPI_HOME`）指向一份**预编好的 rocSHMEM 静态库**（`librocshmem.a`）时才把算子编入；否则加 `-DDISABLE_ROCSHMEM` 把它们**整段编译掉**。镜像里既无 `/opt/rocm/rocshmem`、pip 也不设 `ROCSHMEM_HOME`，故直接 pip 装 `@main` 得到的是**没有 ODC 算子**的包：

```bash
# ✗ 反例：镜像内直接 pip 装 @main（我们在 008/086/087 三节点都复现过）
...
python -c "import primus_turbo.pytorch._C as C; print(hasattr(C,'odc_rocshmem_host'), hasattr(C,'odc_rocshmem_gda'))"
# 实测输出: False False   ← rocSHMEM 未在构建期链接，算子被 -DDISABLE_ROCSHMEM 编掉
```

> （另注：`feat/odc-rocshmem-dist` 这个分支**只在个人 fork 上，AMD-AGI 官方仓库并无此分支**，`pip … @feat/odc-rocshmem-dist` 会直接报 `did not match any file(s)`。所以「换个分支 pip 装」也走不通。）

**正确、且本文全部结果所用的做法：从源码构建 Primus-Turbo，构建期把 `ROCSHMEM_HOME` 指向预编 rocSHMEM，并且单机 / 双机各编一份。** rocSHMEM 本身是**非 pip 的外部依赖**，其 `librocshmem.a` 需针对 host/XGMI-IPC 与 GDA/MLX5 两条路径分别预编（本文分别记作 `rocshmem_single` / `rocshmem_gda`）。在**每个节点的容器内**执行一次：
