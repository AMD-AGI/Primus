...
本节把前文实验的完整复现流程沉淀成可照做、可复制粘贴的教程。要让 ODC 在 ROCm 上**跑得通、跑得对、跑得快**，**三处修复缺一不可**（分散在两个配套 PR 里）：

- **① Primus-Turbo [#409](https://github.com/AMD-AGI/Primus-Turbo/pull/409)——通信本体。** 把 ODC 的 rocSHMEM 单机（host / XGMI-IPC）与多机（GDA，GPU-Direct RDMA）后端并入算子库，暴露为 `primus_turbo.pytorch._C.odc_rocshmem_host` / `odc_rocshmem_gda` 两个 pybind 子模块。**缺了它 ODC 根本无法做 P2P 通信**（`import` 时直接报"请安装带 ODC rocSHMEM ops 的 Primus-Turbo"）。
- **② Primus [#864](https://github.com/AMD-AGI/Primus/pull/864) 内的 `e4577ce`——hook 到正确的 FSDP2 类。** [#808](https://github.com/AMD-AGI/Primus/pull/808) 把 Megatron 的 FSDP2 wrapper 换成了 `PrimusTorchFullyShardedDataParallel`；ODC 若仍 hook 旧类，`reduction_service` 永远是 `None`，**iter2 必崩** `'NoneType' object has no attribute 'clear_accumulations'`。`e4577ce` 改为 hook 新类（带旧类 fallback），且只在 `ODC_ENABLE=1 + use_torch_fsdp2` 下生效，不影响原生 FSDP2 / nccl_pad。
- **③ Primus [#864] 内的 #856 device_id 门控——保住单机速度。** 给 `distributed_init_patches.py` 的 `condition` 追加 `and os.environ.get("ODC_ENABLE","0") != "1"`，让 ODC **跳过** [#856](https://github.com/AMD-AGI/Primus/pull/856) 注入 `device_id`→eager 建 RCCL 通信器的 patch。**缺了它单机会被拖慢 ~6%**：eager-RCCL 的流/DMA 队列会把 ODC 的 XGMI 拷贝流从"与计算重叠"串行化到关键路径（实测跨流重叠 120ms→2.4ms），加速比从 ~1.15× 掉到 ~1.08×。ODC 主通信走 rocSHMEM P2P，本就不需要这些 eager RCCL comm。

三者分工：**Primus 负责算法层**（gather / scatter-accumulate、LB-Mini 变长均衡、per-minibatch 结算，并含上面 ② ③ 两处集成修复），**Primus-Turbo 负责通信算子**（rocSHMEM host/GDA）。PR #864 后 ODC 是 `primus/core/odc/` 下的**纯 Python** in-tree 模块，rocSHMEM 算子从 Primus-Turbo 消费（不再在树内编 ctypes `.so`，`build_rocshmem_backend.sh` 已删除）。

### 6.1 前提条件

**硬件**

- 单机 1.5B：1 台 MI300X ×8，节点内 XGMI 互联即可。
- 双机 14B：2 台 MI300X ×8 = 16 卡，且节点间需 RoCE/RDMA（多张 mlx5 NIC）。**务必选同一 leaf 交换机下、RoCE 互通的相邻节点**——跨拓扑的节点对常在首个跨节点通信处 hang（我们实测多组不相邻节点卡在 GDA warmup，长时间不出 `iteration`）。

**软件**

- ROCm 7.2.0（`gfx942`）、Docker、（可选）SLURM。
- 容器镜像：本项目基准镜像 `tasimage/primus-odc:v26.2`（ROCm 7.2.0 based，`primus/core/odc/README.md` 推荐同款）；读者用等价的 Primus 官方 ROCm 镜像即可。
- HF 离线缓存：预先把模型/数据 stage 到本地，训练时设 `HF_HOME` + `HF_HUB_OFFLINE=1` 全程离线。单机 1.5B 用 `deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B`（模型走默认 `$HF_HOME/hub`，故**不要**覆盖 `HF_HUB_CACHE`）+ SFT 数据 `zai-org/LongAlign-10k`；双机 14B 用对应权重。
...
### 6.3 编译 Primus-Turbo：提供 ODC rocSHMEM 算子（PR #409）

PR #409 把 host/GDA 实现放在 `csrc/kernels/odc_rocshmem/*.cu`，`csrc/pytorch/dist/` 只留 thin pybind wrapper。编译有**两个关键点**：

1. **rocSHMEM 库要先就位**，`ROCSHMEM_HOME` 指向它；多机必须是**打开 `GDA_MLX5`** 的 rocSHMEM build，否则跨节点 GDA 用不了。
2. **device LTO 必须单分区**：`-Xoffload-linker --lto-partitions=1`（PR #409 已在 `setup.py` 内置）。这是我们踩过的最深的坑——默认多分区 device-LTO 会把 rocSHMEM 的 device 默认 context 与 ODC 的 device kernel 拆进不同分区、符号被内部化，导致跨节点 `getmem` 读到 0 → **双机 `grad_norm=0`**。单分区把它们放到一起，才恢复与树内实现一致的双机数值。

```bash
# 容器内
export GPU_ARCHS=gfx942 MAX_JOBS=64
export ROCSHMEM_HOME=/path/to/rocshmem_gda        # 打开 GDA_MLX5 的 rocSHMEM 安装树
...
> **建议单机 / 双机各编一份 Turbo。** 单机走 host/XGMI-IPC（`odc_rocshmem_host`），双机走 GDA（`odc_rocshmem_gda`）；两者依赖的 rocSHMEM 连接后端不同，单机 IPC 的对称堆与双机 GDA 混用时容易在 symmetric-heap 上打架。我们的做法是保留两个目录——`Primus-Turbo`（`ROCSHMEM_HOME=rocshmem_gda`，多机，带 lto=1）与 `Primus-Turbo-single`（`ROCSHMEM_HOME=rocshmem_single`，单机 IPC），运行时用 `PRIMUS_TURBO_PATH` 分别指向。说白了，`Primus-Turbo-single` 不是什么独立组件，**就是第二份 Turbo 副本、只不过 link 到单机 rocSHMEM 构建**；这纯是命名 / 便利上的取舍、**并非强制**——只跑单机时留一份 `single` 即可，只跑双机时留一份 `gda` 即可。

### 6.4 Primus 侧（PR #864）：不再编 .so，只需能 import 算子

PR #864 后 ODC 是纯 Python，**不再需要** `build_rocshmem_backend.sh`。只要让 `import primus_turbo`（含 ODC 算子）可解析即可——`run_odc.sh` 会把 `PRIMUS_TURBO_PATH` 前置到 `PYTHONPATH`。再准备好 HF 模型/数据缓存（`HF_HOME`、`HF_HUB_OFFLINE=1`）。
...
- **第 1 参数**选 ODC 的 P2P 后端：`rocshmem`＝**本文主用、已验证**的后端（我们移植的 rocSHMEM 单边 p2p，单机 host/XGMI-IPC、双机 GDA 均走它）；`mori`（MORI-SHMEM）为**遗留 / 优先级低**选项——**其双机（多机）实现有已知 bug，切勿用于双机**，本文所有 ODC 结果都用 `rocshmem`。`run_odc.sh` 默认 `export ODC_ENABLE=1`，因此基线臂要显式追加 `ODC_ENABLE=0` 才回退到标准 FSDP2 + RCCL；**注意此时第 1 个位参（后端）被完全忽略**——ODC 已整体关闭，跑的就是标准 Megatron FSDP2 + RCCL，与 `mori` / “mori pad” **毫无关系**（见下表）。
...
```bash
# 容器内，单机 8 卡
export PRIMUS_TURBO_PATH=/workspace/code/Primus-Turbo-single   # 单机 IPC 版
...
**第一步——确认三处修复都生效。** 在训练日志里按顺序找到这些行（均来自真实代码的 `log_rank_0`）：

1. **#856 device_id 门控生效**（修复 ③）——ODC 臂应打印：

   ```
   [Patch] ⊘ Skipped: megatron.distributed.init_process_group_device_id (condition not met)
   ```

   因为 `ODC_ENABLE=1` 使 `condition` 里 `... != "1"` 为假 → 跳过 eager-RCCL。**对照组 nccl_pad 则应看到 `[Patch] ✓ Applied: megatron.distributed.init_process_group_device_id`**（它本就要用这些 RCCL comm 做 reduce-scatter）。若 ODC 臂也出现 `✓ Applied`，说明 gate 未生效，速度会掉到 SLOW 簇。
...
2. **ODC hook 到正确的 FSDP2 类**（修复 ②）：
   ...
   关键是类名必须是 **`PrimusTorchFullyShardedDataParallel`**（不是旧的 `TorchFullyShardedDataParallel`）；若崩在 iter2 报 `NoneType ... clear_accumulations`，即 `e4577ce` 未生效。

3. **训练稳定推进**：`lm loss` 随 iter **下降**（1.5B/LongAlign 首步 ~11.x，正常 run iter200 loss ≈ 10.18）；**0 nan / 0 crash**；稳态 `ms/step` 平稳。
...
**第二步——算加速比。** 每个 run 的日志会打印 `iteration N/M | ... | elapsed time per iteration (ms): X`。取稳定段（跳过前几步 warmup）的 `ms/iter` 中位数，按

> 加速比 = `NCCL_pad 的 ms/iter` ÷ `本 run 的 ms/iter`（>1 即比 RCCL 快）
...
- **`import primus_turbo` 报错 / 无 `odc_rocshmem_host`（修复 ①）**：多半是镜像自带的旧 turbo（`DISABLE_ROCSHMEM`）抢先被 import，或 `PRIMUS_TURBO_PATH` 没生效。先跑 `container_prep.sh` 把镜像自带 turbo 从 site-packages 挪走，再确认 `python -c "import primus_turbo; print(primus_turbo.__file__)"` 落在你 6.3 构建的树；`hasattr(C,'odc_rocshmem_host')==False` 则说明构建时 rocSHMEM 没编进来（`find_rocshmem_library()` 返回 None → 自动加 `-DDISABLE_ROCSHMEM`），回 6.3 检查 `ROCSHMEM_HOME`/`MPI_HOME`，**不要**显式设 `DISABLE_ROCSHMEM`。
- **iter2 崩 `'NoneType' object has no attribute 'clear_accumulations'`（修复 ②）**：hook 了旧类。确认 Primus 为含 `e4577ce` 的分支，日志应出现 `hooked PrimusTorchFullyShardedDataParallel.__init__`（而非旧的 `TorchFullyShardedDataParallel`）。
- **能跑但单机慢 ~6%（ms/step ~2270+，修复 ③）**：#856 gate 未生效 / `ODC_ENABLE` 未置 1。确认 `run_odc.sh` 设了 `ODC_ENABLE=1`，且日志里 device_id patch 为 `⊘ Skipped ...(condition not met)` 而非 `✓ Applied`。
- **首步就 NaN（rocshmem 后端）**：复用了异构 toolchain 的 Triton cache，加载了不匹配的 device kernel。`run_odc.sh` 已对 rocshmem 默认每次用全新 `TRITON_CACHE_DIR`，勿手动 pin 到旧 cache。
- **双机 `grad_norm=0`**：device-LTO 多分区所致，务必 `-Xoffload-linker --lto-partitions=1`（PR #409 已内置）。
- **单双机 Turbo 分开编**：单机 IPC 与双机 GDA 的 rocSHMEM 对称堆 / 连接后端不同，混用会在 symmetric-heap 报错；用两份 `PRIMUS_TURBO_PATH` 隔离。
- **env 命名要一致**：设备核 knob 已统一为 `PRIMUS_TURBO_ODC_GDA_{BLOCK,PIPE,NUM_QP}`（PR #409），Primus 消费端（`scatter_accumulate.py` 的 PIPE）同步改名（PR #864）；两边同名才生效。跑本文默认值即可，无需显式设。
- **节点选择**：双机务必选 RoCE 互通的相邻节点；不相邻的节点对常卡在首个跨节点 GDA warmup（长时间无 `iteration`）。可先用 `nccl_pad` 短跑几步做连通性 smoke——能稳定出 `iteration` 即说明 RoCE 通。
