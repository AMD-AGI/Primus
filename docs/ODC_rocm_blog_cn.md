...
- **① Primus-Turbo [#409](https://github.com/AMD-AGI/Primus-Turbo/pull/409)——通信本体。** 把 ODC 的 rocSHMEM 单机（host / XGMI-IPC）与多机（GDA，GPU-Direct RDMA）后端并入算子库，暴露为 `primus_turbo.pytorch._C.odc_rocshmem_host` / `odc_rocshmem_gda` 两个 pybind 子模块。**缺了它 ODC 根本无法做 P2P 通信**。
...
# Primus-Turbo（含 ODC rocSHMEM 算子，PR #409）——单机 IPC 版与双机 GDA 版各一份
...
### 6.3 让 Primus-Turbo 的 GDA 算子就位（关键：`.image_bak` 去影子）

镜像 `tasimage/primus-odc:v26.2` 自带一份**没有 ODC rocSHMEM 算子的 stock `primus_turbo`**（装在 site-packages）。由于 `run_pretrain.sh` 的 `setup_pythonpath()` 会把 site-packages 排在 `PRIMUS_TURBO_PATH` 前面，这份 stock 包会**抢先 import、遮住**你放在 NFS 上的 GDA 构建。解决办法是把镜像自带的那份**改名让它失效**（`.image_bak`），让 `PYTHONPATH` 上的 GDA 版胜出：

```bash
# 容器内
SP=$(python -c "import sysconfig;print(sysconfig.get_paths()['purelib'])")
# 把镜像自带的 stock primus_turbo（无 GDA 算子）改名令其失效
...
# 用你的 GDA 版 Turbo 验证算子在位（$TURBO = 编好的 Primus-Turbo 树）
TURBO=/path/to/Primus-Turbo
PYTHONPATH="$TURBO:$SP" python -c \
  "import primus_turbo.pytorch._C as C; print('C=', C.__file__); \
print('has_host=', hasattr(C,'odc_rocshmem_host'), 'has_gda=', hasattr(C,'odc_rocshmem_gda'))"
# 期望: has_host=True has_gda=True
```

> **单机 / 双机各留一份 Turbo。** 单机走 host/XGMI-IPC（`odc_rocshmem_host`），双机走 GDA（`odc_rocshmem_gda`）；两者依赖的 rocSHMEM 连接后端不同，单机 IPC 的对称堆与双机 GDA 混用时容易在 symmetric-heap 上打架。我们的做法：`Primus-Turbo-single`（单机 IPC，link 到单机 rocSHMEM 构建）与 `Primus-Turbo`（双机 GDA，`--lto-partitions=1`），运行时用 `PRIMUS_TURBO_PATH` 分别指向。只跑单机时留一份 `single` 即可，只跑双机时留一份 `gda` 即可。
...
- **`import primus_turbo` 报错 / 无 `odc_rocshmem_*`（修复 ①）**：多半是镜像自带的 stock turbo（无 GDA 算子）抢先被 import，或 `PRIMUS_TURBO_PATH` 没生效。按 6.3 把镜像自带的那份改名 `.image_bak`，确认 `python -c "import primus_turbo; print(primus_turbo.__file__)"` 落在你的 GDA 构建树、`hasattr(C,'odc_rocshmem_gda')==True`。
