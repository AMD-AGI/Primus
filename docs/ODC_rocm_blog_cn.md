...
> 2. **GDA 算子就位**（6.3）：`.image_bak` 去掉镜像自带的无 GDA turbo，验证 `has_gda=True`。
...
> **重要变化——ODC 现在由「配置项」驱动，不再靠环境变量。** 早期版本用 `ODC_ENABLE=1` / `ODC_PHASE=2` / `LB_MINI_SAME_MICRO=0` 这类 env 开关功能；PR [#864](https://github.com/AMD-AGI/Primus/pull/864)（分支 `feat/odc-consume-turbo`，本文基于 HEAD `ac34bbc3`）做了一次重构，把**所有正式功能开关都改成了 Primus 的 yaml 配置项**（`enable_odc`、`odc_phase`、`enable_odc_lb_mini`、`odc_p2p_backend`、`odc_rocshmem_gda`、`odc_gda_*` …）。ODC 库自身的调优 knob 由 `odc.runtime_config` 在 `before_train` 阶段从配置读入，**运行时不再读 `os.environ`**。因此本节所有命令都以「改配置项」为准；遗留的 `ODC_*` 环境变量已失效（脚本里若还残留 `export ODC_P2P_BACKEND=...` 只是历史遗留、不再影响功能）。

要让 ODC 在 ROCm 上**跑得通、跑得对、跑得快**，**三处修复缺一不可**：

- **① Primus-Turbo——通信本体。** 把 ODC 的 rocSHMEM 单机（host / XGMI-IPC）与多机（GDA，GPU-Direct RDMA）后端并入算子库，暴露为 `primus_turbo.pytorch._C.odc_rocshmem_host` / `odc_rocshmem_gda` 两个 pybind 子模块。**缺了它 ODC 根本无法做 P2P 通信**。这套算子**现已并入 [Primus-Turbo](https://github.com/AMD-AGI/Primus-Turbo) 官方 `main`**（`csrc/kernels/odc_rocshmem/`，且 `setup.py` 已内置 GDA 必需的 `--lto-partitions=1`），因此**直接 `pip` 装官方 Primus-Turbo 即含，无需特殊分支或手写编译流程**（安装时本地自动编译 HIP kernel，见 6.3）。
- **② hook 到正确的 FSDP2 类。** [#808](https://github.com/AMD-AGI/Primus/pull/808) 把 Megatron 的 FSDP2 wrapper 换成了 `PrimusTorchFullyShardedDataParallel`；ODC 若仍 hook 旧类，`reduction_service` 永远是 `None`，**iter2 必崩** `'NoneType' object has no attribute 'clear_accumulations'`。PR #864 改为 hook 新类（带旧类 fallback），且**只在 `enable_odc: true` + `use_torch_fsdp2: true` 时生效**（补丁 `condition` 就是这两项配置项），对原生 FSDP2 / `nccl_pad` 零影响。
- **③ #856 device_id 门控——保住单机速度。** `distributed_init_patches.py` 的补丁 `condition` 现在写成 `use_torch_fsdp2 and not enable_odc`：即 **ODC 臂（`enable_odc: true`）会自动跳过** [#856](https://github.com/AMD-AGI/Primus/pull/856) 注入 `device_id`→eager 建 RCCL 通信器的 patch。**缺了它单机会被拖慢 ~6%**：eager-RCCL 的流/DMA 队列会把 ODC 的 XGMI 拷贝流从"与计算重叠"串行化到关键路径（实测跨流重叠 120ms→2.4ms），加速比从 ~1.15× 掉到 ~1.08×。ODC 主通信走 rocSHMEM P2P，本就不需要这些 eager RCCL comm。这里的门控靠的是 `enable_odc` **配置项**（不再是 `ODC_ENABLE` 环境变量）。

三者分工：**Primus 负责算法层 + 集成**（gather / scatter-accumulate、LB-Mini 变长均衡、per-minibatch 结算，并含上面 ② ③ 两处集成修复，**全部由配置项门控**），**Primus-Turbo 负责通信算子**（rocSHMEM host/GDA）。PR #864 后 ODC 是 `primus/core/odc/` 下的**纯 Python** in-tree 模块，rocSHMEM 算子从 Primus-Turbo 消费（不再在树内编 ctypes `.so`，`build_rocshmem_backend.sh` 已删除）。
...
# 通信算子来自 Primus-Turbo：它是 pip 依赖，官方 main 已内置 ODC rocSHMEM host/GDA 算子，
# 在 6.3 用 pip 安装即可（安装时本地自动编译，无需特殊分支）
...
### 6.3 装含 ODC 算子的 Primus-Turbo（官方 `main` 已内置，pip 装即可）

ODC 的 rocSHMEM host/GDA 算子**已并入 Primus-Turbo 官方 `main`**，直接用 pip 安装即可——**不需要特殊分支、也不需要你手写编译流程**。官方索引当前以 **sdist** 分发，安装时会在本机**自动编译** HIP kernel（含 ODC 算子与 GDA 必需的 `--lto-partitions=1`）；需 ROCm 工具链、支持 `gfx942`/`gfx950`，约几~十几分钟。在**每个节点的容器内**装一次：

```bash
# 容器内（用镜像自带的 ROCm torch，故 --no-build-isolation，避免 pip 拉非 ROCm torch）
GPU_ARCHS="gfx942;gfx950" pip3 install --no-build-isolation --force-reinstall \
  "git+https://github.com/AMD-AGI/Primus-Turbo.git@main"
# 或从源码装：
#   git clone https://github.com/AMD-AGI/Primus-Turbo.git && cd Primus-Turbo
#   pip3 install -r requirements.txt
#   GPU_ARCHS="gfx942;gfx950" pip3 install --no-build-isolation ".[pytorch]"

# 验证 ODC 算子在位（host=单机 XGMI-IPC，gda=多机 GPU-Direct）
python -c "import primus_turbo.pytorch._C as C; \
print('has_host=', hasattr(C,'odc_rocshmem_host'), 'has_gda=', hasattr(C,'odc_rocshmem_gda'))"
# 期望: has_host=True has_gda=True
```

- `--force-reinstall` 用来覆盖镜像 `tasimage/primus-odc:v26.2` 里可能自带的**旧版 stock `primus_turbo`**（无 ODC 算子）。
- 装好后**单机 / 双机共用同一份包**（`host` 与 `gda` 两套算子都在）；运行时走哪条后端由**配置项 `odc_rocshmem_gda`** 决定（`false`＝单机 host / `true`＝多机 GDA），**无需分别装两份**。
- 这样装进 site-packages 后，后文 6.5 / 6.6 里的 `PRIMUS_TURBO_PATH=...` 就**可以省略**（包已在环境里，`import primus_turbo` 直接命中）。

> **兜底（仅当你坚持用 NFS 上的自建 dev 树、而非 pip 装）**：若用 `PYTHONPATH` 指向一份自编的 Primus-Turbo 树，镜像自带的 site-packages 版会**抢先 import 遮住**它。把自带那份改名失效即可，再让 `PYTHONPATH` 上的胜出：
> ```bash
...
> # 之后 PRIMUS_TURBO_PATH=<你的Turbo树> 生效
> ```
> **推荐直接 pip 装官方版**（上面），省去这套影子处理。
...
- `run_odc.sh` 的第 2 个位参 `pad|nopad` **已是 no-op**（历史遗留）——旧的"对齐 vs 解耦"A/B 开关已删除，微批对齐模式现由 `enable_odc` 自动派生（`true`→解耦/nopad，`false`→对齐）。

统一入口 `run_odc.sh`（单机 / 双机同一脚本）：

```
run_odc.sh <mori|rocshmem> <pad|nopad> <exp_yaml_relpath> <exp_name> [KEY=VAL ...]
```

- **第 1 位参**（`rocshmem`/`mori`）现在只负责铺 rocSHMEM 的**运行时基础设施 env**（`ROCSHMEM_HEAP_SIZE`＝8 GiB 纯字节、`ROCSHMEM_BOOTSTRAP_SOCKET_IFNAME`、每次 run 新建 `TRITON_CACHE_DIR` 防复用不匹配 device kernel → NaN）；**真正选后端的是 yaml 的 `odc_p2p_backend`**——两者保持一致（都 rocshmem）即可。脚本里残留的 `export ODC_P2P_BACKEND=...` 已不再影响功能。
- **第 2 位参** `pad|nopad` 已是 no-op（见上）。
- 其余 `KEY=VAL` 仍会被逐个 `export`，用于传集群 / GDA 的**基础设施** env（NCCL/ROCSHMEM 等）；**ODC 功能开关不要再从这里传**，改到 yaml。
- 脚本还会把 `PYTHONPATH` 铺好（含 `odc_early` 载序修复 shim 与 `PRIMUS_TURBO_PATH`），随后调 `examples/run_pretrain.sh`（`torchrun ... train pretrain --config <EXP>`）。**注意 `run_odc.sh` 不透传 CLI 覆盖**，故要改 `global_batch_size`、`enable_odc` 等都得改 yaml。
...
双机在**每个节点各起一个 rank 组**（NODE_RANK 0/1，各 8 卡 → 共 16 rank），rocSHMEM 用 **uid-over-socket** bootstrap（PR #864 后不再用 MPI，纯 torchrun 起）。推荐用 `primus-cli slurm` 一行 fan-out 到两节点，各自 `docker run --rm` 起全新容器再驱动 `torchrun`——**无需常驻容器、无需 `rank_node.sh`**。
...
`primus-cli slurm srun` 里三段 `--` 依次分隔：**slurm/srun 参数** ｜ **docker/容器参数（含全部 `--env`）** ｜ **`train pretrain` 训练参数（含 ODC 配置项覆盖）**。`slurm-entry` 会自动把 `MASTER_ADDR/MASTER_PORT/NNODES/NODE_RANK/GPUS_PER_NODE` 注入每个节点，无需手工传。

**对照 `nccl_pad` 对齐基线**：把最后一段的 ODC 覆盖换成 `--enable_odc false --enable_odc_lb_mini true`（去掉 `--odc_p2p_backend`/`--odc_rocshmem_gda`，其余不变），即标准 FSDP2 + RCCL。⚠️ **`--enable_odc_lb_mini true` 不可省**：`qwen14B-odc-dn.yaml` 本就带 `enable_odc_lb_mini: true`，`enable_odc: false` 时它自动切到**对齐模式**，喂给基线**与 `odc_nopad` 完全相同的 LB-Mini 变长数据**（只是各 rank 微批数被 `all_reduce(MAX)` 对齐、RCCL-safe）——这才是公平口径。漏了它则基线跑更重的标准 padded 数据、加速比虚高（见 6.7 的 ~1.9× 假象）。

**几处 `--env`/`--`覆盖的一句话说明**：

- `--odc_p2p_backend rocshmem`：选 rocSHMEM 单边 P2P（非 mori）。
- `--odc_rocshmem_gda true`：切到多机 GPU-Direct RDMA 路径（`odc_rocshmem_gda`），而非单机 host/XGMI-IPC。
- `ROCSHMEM_HCA_LIST=$HCA`：RoCE NIC 白名单（本集群 8 张 mlx5，`mlx5_1/_6` 是 eth 管理口需排除）。
- `ROCSHMEM_BOOTSTRAP_SOCKET_IFNAME=eth0`：rocSHMEM 走 **uid-over-socket** bootstrap（ODC 自带、不用 MPI）在 eth0 上交换 uid。
...
- **注意（`slurm-entry` 的 MASTER_ADDR）**：`slurm-entry` 会强制 `MASTER_ADDR=`短主机名。当 `/etc/hosts` 里 `127.0.1.1` 那行被注释掉时工作正常；若集群把主机名映射到 loopback（`127.0.1.1 <hostname>`），rendezvous 会绑到本地回环 → 跨节点连不上，此时改用下方 `srun --overlap` 兜底路径更稳。

#### 稳健兜底：`srun --overlap` + `docker exec` + `run_odc.sh`

若集群主机名映射到 loopback、或想复用常驻容器里预置的环境，可退回 **`srun --overlap` + `docker exec` + `run_odc.sh`** 编排（每节点各送进常驻容器一个 rank）：两节点分别执行
...
并各自 `export NNODES=2 GPUS_PER_NODE=8 NODE_RANK=<0|1> MASTER_ADDR=<nodeA> MASTER_PORT=<port>`。**注意**：这里 `KEY=VAL` 只传**基础设施** env；ODC 功能开关（含后端 `rocshmem`、GDA）仍看 yaml——所以此路径也要给 `qwen14B-odc-dn.yaml` 补上 `odc_p2p_backend: rocshmem` + `odc_rocshmem_gda: true`。**对齐基线**同法：复制一份 yaml、设 `enable_odc: false` + **保留 `enable_odc_lb_mini: true`**（喂同一份变长数据），用它跑 `run_odc.sh ... <baseline_yaml> nccl_pad ...`。封装样例见 `full_exp/matrix_dual_harness/`。它对 hostname/loopback 更鲁棒；但**首选、已验证**的双机路径是上面的 `primus-cli slurm`。

### 6.7 判断成功、读结果、算加速比

**第一步——确认三处修复都生效。** 在训练日志里找到这些行（均来自真实代码的 `log_rank_0`）：

1. **ODC 运行时配置已从 yaml 读入**（证明配置项生效）：

   ```
   [ODC.torch_fsdp2] runtime config populated from trainer config (p2p_backend=rocshmem, rocshmem_gda=True, warmup_mode=strided)
   ```

   `p2p_backend` 必须是 **`rocshmem`**（双机 `rocshmem_gda=True`）；若显示 `mori`，说明 `odc_p2p_backend` 没设对。

2. **ODC hook 到正确的 FSDP2 类**（修复 ②）：
   ...
   类名必须是 **`PrimusTorchFullyShardedDataParallel`**（不是旧的 `TorchFullyShardedDataParallel`）；若崩在 iter2 报 `NoneType ... clear_accumulations`，即 hook 了旧类。

3. **#856 device_id 门控生效**（修复 ③）——ODC 臂应打印：

   ```
   [Patch] ⊘ Skipped: megatron.distributed.init_process_group_device_id (condition not met)
   ```

   因为 `enable_odc: true` 让补丁 `condition`（`use_torch_fsdp2 and not enable_odc`）为假 → 跳过 eager-RCCL。**对照组 nccl_pad 则应看到 `[Patch] ✓ Applied: megatron.distributed.init_process_group_device_id`**（它本就要用这些 RCCL comm 做 reduce-scatter）。若 ODC 臂也出现 `✓ Applied`，说明配置没设对（`enable_odc` 没真正到 true），速度会掉到 SLOW 簇。

4. **训练稳定推进**：`lm loss` 随 iter **下降**（1.5B/LongAlign 首步 ~11.x）；**0 nan / 0 crash**；稳态 `ms/iter` 平稳。

快速抓取：

```bash
grep -E "runtime config populated|hooked PrimusTorchFullyShardedDataParallel|⊘ Skipped: .*device_id|✓ Applied: .*device_id" "$LOG"
...
|---|---|---|---|---|
| 双机 14B，gbs128，50 iter | ≈ **71.2k** ms/iter（≈ env 版 71.7k） | ≈ **80.2k** ms/iter | ≈ **1.13×** | 逐步下降、0 nan |
| 单机 1.5B，gbs16，200 iter | ≈ **2.1k** ms/iter | ≈ **2.5k** ms/iter | ≈ **1.19–1.21×** | 收敛至 ~10.0、0 nan |

> **口径校正（重要）**：配置项化后的代码经硬件验证与早期 env-based 修复**行为等价**（双机 + 单机均 `nan=0`、量级一致）。此前对**非对齐** nccl 基线一度看到 ~1.9× 的加速，那是**口径假象**——基线跑了更重的标准 padded 数据；本文改用 `enable_odc_lb_mini: true` 的**对齐同数据**基线后已纠正，真实加速比即上表的 ~1.13×（双机）/ ~1.2×（单机）。跨节点是 ODC 的主场——设备越多、越跨节点、负载越不均衡，ODC 越赚。

### 6.8 踩坑清单（强烈建议先读）

- **`import primus_turbo` 报错 / 无 `odc_rocshmem_*`（修复 ①）**：多半是镜像自带的 stock turbo（旧版、无 ODC 算子）在用。按 6.3 用 `pip install --force-reinstall "git+https://github.com/AMD-AGI/Primus-Turbo.git@main"` 装官方版（含 ODC 算子），确认 `hasattr(C,'odc_rocshmem_host')==True` 且 `hasattr(C,'odc_rocshmem_gda')==True`。若坚持用自建 dev 树，则用 `.image_bak` 去影子 + `PRIMUS_TURBO_PATH`（见 6.3 兜底）。
...
- **primus-cli 直连三坑**：① `--volume` 必须写 `src:dst`（裸路径会建匿名空卷盖住代码）；② `PYTHONPATH` 必须整条注入（primus-cli 跳过 `setup_pythonpath`，漏了就 `import odc`/`import primus_turbo` 失败）；③ 内置 pretrain hook 缺 `stage==sft` skip，用预 `touch` 的 `.done`（指到 `TOKENIZED_DATA_PATH`）让它跳过 prep。
- **节点选择**：双机务必选 RoCE 互通的相邻节点；不相邻的节点对常卡在首个跨节点 GDA warmup（长时间无 `iteration`）。可先用 `enable_odc: false` 短跑几步做连通性 smoke——能稳定出 `iteration` 即说明 RoCE 通。
...
- **gbs16 偶发不收敛（loss 卡 ~11.95）**：训练稳定性问题，与 ODC 无关，`nccl_pad` 也会中招；换 `seed` 或跳过该 run 即可。
