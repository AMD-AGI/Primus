...
| `turbo_gemm_backend`   | `default`       | v0.8.0  | Pin the Primus-Turbo dense GEMM backend instead of letting Primus-Turbo choose. | NA (writes `PRIMUS_TURBO_GEMM_BACKEND` from the turbo linear constructors) | Takes the env var's syntax: one backend for every precision (`ck`) or a per-precision list (`fp8:ck,other:hipblaslt`). See `primus/configs/modules/megatron/primus_turbo.yaml` for the backends currently available per precision. Any value other than `default` overwrites a variable inherited from the shell. **Please set `enable_primus_turbo=True` first.** |
| `turbo_grouped_gemm_backend`   | `default`       | v0.8.0  | Pin the Primus-Turbo MoE grouped GEMM backend. | NA (writes `PRIMUS_TURBO_GROUPED_GEMM_BACKEND` from the turbo grouped linear constructors) | Same syntax as `turbo_gemm_backend`. **Please set `enable_primus_turbo=True` and `use_turbo_grouped_gemm=True` first.** |
| `turbo_moe_dispatch_combine_backend`   | `default`       | v0.8.0  | Pin the Primus-Turbo MoE dispatch/combine backend. | NA (writes `PRIMUS_TURBO_MOE_DISPATCH_COMBINE_BACKEND` from `PrimusTurboDeepEPTokenDispatcher.__init__`) | The variable is written during model construction, so the `expert_model_parallel_size >= 16` / `turbo_deepep_num_cu` check in `rocm_arg_validation.py` still reads whatever the shell exported. Hooks such as `runner/helpers/hooks/05_using_uep.sh` set the same variable. **Please set `enable_primus_turbo=True` and `use_turbo_deepep=True` first.** |
