...
**File**: Launch with `runner/primus-cli` and the appropriate config
...
def main():
    # Launch via runner/primus-cli with a config from examples/megatron/configs/MI300X/diffusion/
...
5. `runner/primus-cli` - Launch with a config from `examples/megatron/configs/MI300X/diffusion/`
