#!/bin/bash
#
# DSv3 8-node OVERLAP validation (selective recompute + cross-microbatch overlap)
#
# Key differences vs baseline (start_training_dsv3_baseline_8node.sh):
#   - overlap_moe_expert_parallel_comm=True   (via YAML)
#   - recompute_granularity=selective         (via YAML, required by overlap)
#   - turbo_sync_free_moe_stage=0             (via YAML, disabled for overlap compat)
#   - mtp_num_layers=null                     (via YAML, allows PP=8 + overlap)
#   - MBS=1                                   (halved to fit selective recompute)
#
# With 8 nodes, EP=8 spans all 8 nodes → inter-node AllToAll via AINIC/InfiniBand
# This is the scenario where overlap hides ~100-200µs A2A behind expert GEMM.
...
export SLURM_NODELIST="uswslocpm2m-106-[2244-2250,2252]"
...
export MBS=1    # selective recompute + DSv3 on 8 nodes requires MBS=1 to fit in 288GB GPU memory
export PRIMUS_SEQ_LENGTH=2048  # reduced from 4096 to give headroom past iteration 1 (98%+ memory at 4096)
export GBS=$((128 * NNODES))   # 1024 (same total tokens per step)
export PRIMUS_TOTAL_LAYERS=48  # reduced from 61 to fit in memory with selective recompute + overlap microbatch
...
# Uses deepseek_v3-FP8-moe_overlap.yaml which sets:
...
export PRIMUS_EXP_NAME=dsv3-moe_overlap_v3-${PRETRAIN_TYPE}-PP${PRIMUS_PP}-EP${PRIMUS_EP}-MBS${MBS}
