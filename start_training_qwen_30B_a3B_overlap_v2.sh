#!/bin/bash
#
# Validation: Qwen3-30B-A3B MoE EP A2A overlap (PP=1)
#
# vs baseline: start_training_qwen_30B_a3B.sh
#   Baseline: ~full recompute, no overlap
#
# Change from baseline:
#   - overlap_moe_expert_parallel_comm=True (in YAML)
#   - recompute_granularity=selective (in YAML, required by overlap)
#   - turbo_sync_free_moe_stage=0 (disabled, overlap subsumes it)
#
# Nodes: amd-aig-2 partition (same as Qwen baseline)
...
export SLURM_NODELIST="uswslocpm2m-106-2165"
...
export PRIMUS_EXP_NAME=qwen3_30B_A3B-overlap_v2-${PRETRAIN_TYPE}-PP${PRIMUS_PP}-EP${PRIMUS_EP}
