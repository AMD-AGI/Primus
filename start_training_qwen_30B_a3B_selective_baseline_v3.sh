#!/bin/bash
#
# Qwen3-30B-A3B selective baseline (PP=1, GBS=64 = 8 micro-batches of MBS=8)
# Identical to overlap_v3 EXCEPT overlap_moe_expert_parallel_comm=False
...
export SLURM_NODELIST="uswslocpm2m-106-2190"
...
export MBS=1
export GBS=8    # 8 grad-accum steps at MBS=1 (same as overlap for fair comparison)
export PRIMUS_PP=1
export PRIMUS_EP=8
export TURBO_DEEPEEP=True
export LEGACY_GG=True
export PRETRAIN_TYPE=BF16
# Use standard pretrain YAML with selective recompute + no overlap
...
export PRIMUS_EXP_NAME=qwen30b-baseline_selective-MBS${MBS}
