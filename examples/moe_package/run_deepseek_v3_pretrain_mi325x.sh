...
for feature in "${MoE_Features[@]}"; do
	...
	5)
		...
		;;
	6)
		FEATURE_ARGS+=" --pipeline_model_parallel_size 8"
...
if [ $VPP -gt 1 ]; then
	export VPP_ARGS="--num_virtual_stages_per_pipeline_rank $VPP"
fi

if [ "$FP8" = "True" ]; then
	export FP8_ARGS="--fp8 hybrid"
fi
...
if [ "$PROFILE" = "True" ]; then
	...
	PROFILE_ARGS="--profile True \
                  --disable_profiler_activity_cpu ${DISABLE_CPU_TRACE} \
...
export PRIMUS_EXP_NAME="DeepSeekV3_MI325X_FP8${FP8}_MBS${MBS}_GBS${GBS}_SEQ${SEQ_LENGTH}_TP${TP}_ETP${ETP}_PP${PP}_VPP${VPP}_EP${EP}_CP${CP}_Balance${LOAD_BALANCE}_LegacyGG${LEGACY_GG}_Profile${PROFILE}(${PROFILE_STEP_START}-${PROFILE_STEP_END})_NoCPUTrace${DISABLE_CPU_TRACE}"
...
echo "Training Experiment Config: $EXP" | tee -a $LOG_FILE
echo "PRIMUS_EXP_NAME=${PRIMUS_EXP_NAME}" | tee -a $LOG_FILE
