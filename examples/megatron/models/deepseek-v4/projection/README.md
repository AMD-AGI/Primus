...
```
examples/megatron/models/deepseek-v4/projection/
  README.md                      # this file
...
1. Profile each `cr` on MI355X (run inside the training container):
   ...
   CR=0   ./examples/megatron/models/deepseek-v4/projection/script/deepseek_v4_layer_trace-projection.sh
   CR=4   ./examples/megatron/models/deepseek-v4/projection/script/deepseek_v4_layer_trace-projection.sh
   CR=128 ./examples/megatron/models/deepseek-v4/projection/script/deepseek_v4_layer_trace-projection.sh
   ```

2. Build the breakdown JSON from the three traces:

   ```bash
   python3 examples/megatron/models/deepseek-v4/projection/tools/parse_trace.py \
     --model pro \
     --trace cr0=<trace_cr0.json> \
     --trace cr4=<trace_cr4.json> \
     --trace cr128=<trace_cr128.json> \
     --out examples/megatron/models/deepseek-v4/projection/site/data/pro.json
   ```

3. Open the site locally:

   ```bash
   python3 -m http.server -d examples/megatron/models/deepseek-v4/projection/site 8000
