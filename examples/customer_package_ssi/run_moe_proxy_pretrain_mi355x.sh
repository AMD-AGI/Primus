...
if [ -z "${MoE_Features}" ]; then
    # MoE_Features=(0 3)
    # MoE_Features=(0 3 4)
    # MoE_Features=(0 3 5) # bad
    # MoE_Features=(0 3 4 5) # bad
    # MoE_Features=(0 3 4 10)
    # MoE_Features=(0 3 4 10 11)
...
export PRIMUS_EXP_NAME="SSI_MoE_Proxy_MI355X_FP8${FP8}_NNODES${NNODES}_MBS${MBS}_GBS${GBS}_SEQ${SEQ_LENGTH}_MLA${ENABLE_MLA}_MTP${ENABLE_MTP}_REC${RECOMPUTE_LAYERS}_TP${TP}_ETP${ETP}_PP${PP}_VPP${VPP}_EP${EP}_CP${CP}_Balance${LOAD_BALANCE}_LegacyGG${LEGACY_GG}_Profile${PROFILE}-${PROFILE_STEP_START}-${PROFILE_STEP_END}_NoCPUTrace${DISABLE_CPU_TRACE}_Features${FEATURE_TAG}"
...
######################### Training Job #########################
# export PRIMUS_MODEL=ssi_moe_389B.yaml
# export PRIMUS_MODEL=ssi_moe_414B.yaml
export PRIMUS_MODEL=ssi_moe_s2_417B.yaml
export EXP=examples/customer_package_ssi/configs/MI355X/moe_proxy-pretrain.yaml
