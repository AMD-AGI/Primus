...
# docker run -it --name dev_jax_rocm70 --ipc=host --network=host --device=/dev/kfd --device=/dev/dri --device=/dev/infiniband \
# --cap-add=SYS_PTRACE --cap-add=CAP_SYS_ADMIN --security-opt seccomp=unconfined --group-add video --privileged \
# -v ${HOME}:${HOME} "docker.io/rocm/primus:v25.9_gfx950" bash
...
CONFIG="DSv2-Lite-FP8-$FP8.GBS$GBS.PP$PP.EP$EP.CP$CP.VPP$VPP.TOPK$TOPK.rc-$RECOMPUTE_LAYERS.rcids-$RECOMPUTE_ID_START.nodes$NNODES.$OPTIMIZER.BALANCE-$BALANCE-legacygg-$LEGACY_GG-noturboattn-noturbogg"
echo "config: $CONFIG"
...
bash ./examples/run_pretrain.sh --micro_batch_size $MBS \
