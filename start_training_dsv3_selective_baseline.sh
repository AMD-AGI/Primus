#!/bin/bash
#
# DSv3 selective-recompute baseline (no MoE overlap)
# Compares against start_training_dsv3_moe_overlap_v2.sh (same config + overlap=True)
#
# Config vs original log:
#   - recompute: selective (original used full/block/4)
#   - no mtp_num_layers override (YAML default=1)
#   - IDENTICAL to overlap_v2 EXCEPT overlap_moe_expert_parallel_comm=False
...
export SLURM_NODELIST="uswslocpm2m-106-[2180-2187]"
...
export EXP=examples/megatron/configs/MI355X/deepseek_v3-${PRETRAIN_TYPE}-selective_baseline.yaml
export PRIMUS_TEAM=amd
PRIMUS_USER="tas-$(date +%Y%m%d)"
export PRIMUS_USER
export PRIMUS_TOKENIZED_DATA_PATH=/shared_aig/c4/tokenized/c4_en_train_text_document
export PRIMUS_EXP_NAME=dsv3-selective_baseline-${PRETRAIN_TYPE}-PP${PRIMUS_PP}-EP${PRIMUS_EP}
